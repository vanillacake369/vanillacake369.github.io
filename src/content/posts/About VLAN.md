...
그렇다면 VLAN 은 무엇일까? 또 VLAN 동작원리는 어떻게 되는가?
...
VLAN으로 네트워크를 쪼개면 서로 통신이 불가능합니다. 이때 통신이 필요하면 **L3(IP 계층)** 단계에서 길을 찾아줘야 합니다.
...
- nftable/iptable 란 ? 그리고 어떻게 L3 라우터 역할을 하는지?
- OPNsense 란 ? 그리고 어떻게 L3 라우터 역할을 하는지?
...
- **의미:** `ip neigh`에 상대방 MAC이 `REACHABLE`로 떠 있다면, 최소한 **L2(브리지/VLAN) 연결은 정상**이라는 뜻입니다. 만약 `INCOMPLETE`라면 VLAN 설정이나 케이블(Veth) 연결 문제일 확률이 높습니다.
...
1. **네임스페이스 생성:** 독립된 네트워크 방(`ns1`, `ns2`)을 만듭니다.
2. **Veth Pair 생성:** 두 방을 연결할 가상 케이블을 만듭니다.
3. **IP 할당:** 각 방의 인터페이스에 주소를 부여합니다.
4. **연결 확인:** `ping`을 통해 통신을 확인합니다.
5. **확장:** 브리지를 만들고 여기에 `VLAN Filtering`을 활성화하여, 서로 다른 VLAN 태그를 가진 VM들이 격리되는지 테스트합니다.
...
[https://ascentoptics.com/blog/ko/understanding-vlan-what-is-a-vlan-and-how-does-it-work/](https://ascentoptics.com/blog/ko/understanding-vlan-what-is-a-vlan-and-how-does-it-work/)
