...
홈랩과 블로그 저장소를 운영하면서 가장 먼저 자동화한 것이 GitHub Actions 였다. 다만 처음에는 `ubuntu-latest` 위에서 워크플로 하나만 띄워 두는 수준이었고, 운영 옵션들이 각각 어떤 문제를 푸는지는 모호한 상태였다.

그런데 막상 정리하려 하니 모르는 것이 많았다.

- workflow / job / step / runner 의 *경계는 어디인가* — 같은 파일 안에 다 있는데?
- 같은 워크플로 안의 job 들은 *병렬인가 순차인가* — 어떻게 제어하는가?
- secret 은 *fork PR 에서 노출되는가* — 어떤 메커니즘이 차단하는가?
- OIDC 로 *AWS / GCP 자격 증명* 을 가져오는 방식의 보안 모델은?

답은 네 계층 (workflow → job → step → runner) + 큐 + OIDC 토큰 교환 세 가지에 있다.

이 글은 GitHub Actions 의 네 계층과 실행 모델, Secrets 와 GITHUB_TOKEN 의 권한 모델, matrix · cache · reusable workflow · OIDC · self-hosted runner 의 사용 시점, 그리고 실제 운영에서 만난 트러블슈팅과 검증 절차까지 한 번에 정리한다.

# What? 뭘 배움?

## 들어가며 — 네 계층으로 보는 GitHub Actions 🗺️

GitHub Actions 의 구성 요소는 네 계층으로 정리된다. 같은 YAML 파일 안에 다 들어 있지만, 각 계층이 *언제 / 어디서 / 무엇에 의해* 실행되는지는 명확히 분리된다.

```
┌──────────────────────────────────────────────────────────┐
│ 1. Workflow  ──  .github/workflows/X.yml 한 파일         │  ← 트리거 → 작업 묶음
│                  on:, jobs:                              │
├──────────────────────────────────────────────────────────┤
│ 2. Job       ──  jobs.<name>                             │  ← 한 Runner 에 할당
│                  병렬 실행 단위, needs: 로 직렬화         │
├──────────────────────────────────────────────────────────┤
│ 3. Step      ──  jobs.<name>.steps[]                     │  ← 같은 Job 안에서 순차
│                  uses: (액션) 또는 run: (셸 명령)         │
├──────────────────────────────────────────────────────────┤
│ 4. Runner    ──  runs-on:                                │  ← 실행 호스트
│                  github-hosted / self-hosted             │
└──────────────────────────────────────────────────────────┘
```

위에서 아래로 갈수록 *더 좁은 실행 컨텍스트* 다. Workflow 는 한 파일 단위, Job 은 한 러너 단위, Step 은 한 셸 명령 단위로 좁아진다. Runner 는 그 셸 명령이 실제로 떨어지는 호스트다. 다음 mermaid 가 네 계층과 각 계층이 매핑되는 YAML 키를 한 그림으로 묶는다.

```mermaid
flowchart TD
    subgraph WF["Workflow (.github/workflows/ci.yml)"]
        ON["on: push, pull_request<br/>━━━━━━━━━━━━<br/>트리거 정의"]
        subgraph JOBS["jobs:"]
            J1["job: lint<br/>━━━━━<br/>runs-on: ubuntu-latest"]
            J2["job: test<br/>━━━━━<br/>runs-on: ubuntu-latest<br/>needs: [lint]"]
        end
        ON --> JOBS
        J1 -->|needs| J2
    end
    subgraph S1["lint job 의 steps[]"]
        STEP1["- uses: checkout@v4"]
        STEP2["- run: nix-shell ..."]
    end
    subgraph S2["test job 의 steps[]"]
        STEP3["- uses: checkout@v4"]
        STEP4["- run: npm test"]
    end
    J1 -.-> S1
    J2 -.-> S2
    R1["Runner A<br/>(ubuntu-24.04 VM)"]
    R2["Runner B<br/>(ubuntu-24.04 VM)"]
    S1 -.->|runs-on| R1
    S2 -.->|runs-on| R2
```

같은 job 의 step 은 한 러너 위에서 *순차* 실행된다. 다른 job 은 기본적으로 *병렬* 실행되며, 각자 독립된 러너 VM 에서 시작한다. `needs:` 한 줄만 추가하면 두 job 사이에 의존 관계가 생긴다.

본문은 다음 순서로 진행한다. 먼저 CI/CD 도구 지형에서 GitHub Actions 의 위치를 (그 위 절) 정리하고, 네 계층을 코드 예시로 보여준 뒤, 이벤트 → 큐 → 동시성으로 *실행 모델* 을 본다. 그 다음 권한 모델 (Secrets / GITHUB_TOKEN), 운영 옵션 (matrix / cache / reusable), 보안 (OIDC / 핀고정), self-hosted runner 의 사용 시점을 차례로 다룬다.
...
```mermaid
sequenceDiagram
    participant U as 사용자/GitHub
    participant W as Workflow 매칭
    participant Q as Job Queue
    participant R as Runner Pool
    participant L as 워크플로 로그
    U->>W: push / pull_request 이벤트
    W->>W: on: 필터 매칭 (branches, paths)
    W->>Q: 매칭된 워크플로의 job 들 enqueue
    Q->>R: runs-on 라벨에 맞는 러너 요청
    R->>R: hosted runner 프로비저닝<br/>또는 self-hosted 라벨 매칭
    R->>L: steps 순차 실행 + 실시간 로그
    L-->>U: ✅ / ❌ 결과 보고 (commit status)
```
...
세 스코프의 결정 기준은 다음 한 표로 압축된다.

| 스코프 | 노출 범위 | 결정 기준 |
|---|---|---|
| Repository | 한 저장소의 모든 워크플로 | 저장소 하나에서만 쓰고, 환경 구분이 불필요할 때 |
| Environment | 특정 environment 가 지정된 job | staging 과 production 의 키를 분리하거나, 배포 승인 게이트와 묶을 때 |
| Organization | 조직의 허용된 저장소 전체 | 같은 키 (NPM_TOKEN, DOCKERHUB_PASSWORD) 를 여러 저장소가 공유할 때 |

가장 좁은 스코프부터 시작해서 *공유 요건이 명확해진 시점에만* 위로 승격하는 것이 운영 표준이다. 처음부터 Organization 으로 두면 회수 시 영향 범위가 광범위해진다.
...
> [!NOTE]
> **GITHUB_TOKEN vs PAT (Personal Access Token)**
> 두 토큰은 *수명* 과 *scope* 가 다르다. `GITHUB_TOKEN` 은 워크플로 시작 시 GitHub 가 자동 발급해 작업 종료 후 (최대 24시간) 만료된다. 자신의 저장소만 접근할 수 있다. PAT 는 사용자가 명시적으로 만들고 만료 기간도 사용자가 설정한다. cross-repo 접근, 워크플로 파일 자체의 push 같은 *GITHUB_TOKEN 으로 불가능한 작업* 에만 사용한다. 가능하면 PAT 대신 fine-grained PAT 또는 OIDC 로 대체한다.
...
```mermaid
sequenceDiagram
    participant W as 워크플로 (Step)
    participant G as GitHub OIDC Provider<br/>(token.actions.githubusercontent.com)
    participant I as 클라우드 IAM<br/>(AWS STS / GCP STS)
    participant C as 클라우드 리소스<br/>(S3 / GCS)
    W->>G: id-token: write 권한으로 JWT 요청
    G-->>W: 단기 JWT (iss / sub / aud claim 포함)
    W->>I: AssumeRoleWithWebIdentity(JWT)
    I->>I: JWT 검증<br/>sub: repo:OWNER/REPO:ref:...<br/>신뢰정책 매칭
    I-->>W: 임시 자격증명 (1시간)
    W->>C: aws s3 sync / gcloud storage cp
    C-->>W: ✅ 결과
```

핵심은 *JWT 의 sub claim 이 워크플로의 신원 그 자체* 라는 점이다. IAM 신뢰정책에 `sub: repo:OWNER/REPO:ref:refs/heads/main` 같은 조건을 박으면 *특정 저장소의 특정 브랜치* 에서만 자격증명이 발급된다. 장기 access key 를 secret 에 저장하지 않아도 같은 결과를 얻을 수 있다.
