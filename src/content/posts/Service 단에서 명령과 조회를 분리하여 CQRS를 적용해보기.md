---
description: “단일장애지점 극복을 위해 Service 레이어에서 Command와 Query를 분리하여 CQRS 패턴을 직접 적용한 경험을 정리했다.”
date: 2024-02-11
tags: [journal]
lang: ko
draft: false
...
현재 Master db 인스턴스의 snapshot이 생성. 2.

생성된 snapshot을 이용하여 다른 AZ에 대기 인스턴스가 생성. 3.
...
모든 소스 코드는 [여기](https://github.com/Team-BC-1/gream)를 보면 된다.
...
이제 Config 클래스를 만들어서 DataSource 를 달리 매핑해줄 차례이다.
...
예시 코드를 간략하게 설명하자면 다음과 같다.

BuyProvider 내에서는 다음과 같은 서비스가 사용된다.
...
SPOF, Primary/Secondary 에 대한 핸들링을 오롯히 AWS에게 맡겼다.

이러한 점은 여러 부작용을 야기한다. - Vendor Lock-In : 모든 리소스를 AWS에 의존하여 AWS Vendor 에 Lock-In 되는 현상을 말한다.

상황이나 비용에 맞춰 다른 Cloud Service 사용하거나 커스텀할 수 있어야 하는데 그러지 못 하게 된다. - 굉장한 비용부담 : RDS 자체적으로도 굉장히 비싼 편에 속한다.

실제로 100$/1m 중 30%가 RDS와 ELB 였다.

부하테스트"만" 돌렸는데도 이 정도라면 -- 100만건도 안 돌렸다, 최대 5만건이 전부였다 -- 실제 서버를 돌린다면 정말 엄청난 서버비를 낭비하게 될 것이다.
...
만약 [아래와 같은 Aggregate](https://www.youtube.com/watch?v=704qQs6KoUk&t=614s&ab_channel=%EC%9A%B0%EC%95%84%ED%95%9C%ED%85%8C%ED%81%AC) 를 조회해야한다고 치자.

Query Model 을 분리하는 게 필연적일 것이다.
