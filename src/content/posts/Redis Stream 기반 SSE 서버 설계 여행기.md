---
title: "Redis Stream 기반 SSE 서버 설계 여행기"
...
- 성공한 서버(AppB)가 Leader가 되고 TTL 만료 전까지 `PEXPIRE` 또는 `SET ... XX PX` 방식으로 갱신(Heartbeat)
...
1. **리더 여부 확인 (`isLeader`)**
    1. 현재 서버가 리더인지 Redis에서 확인
2. **리더라면 TTL 연장 (`renewLeader`)**
    1. 리더라면 Redis Key의 TTL을 갱신하여 리더 권한을 유지
3. **리더가 아니라면 리더 획득 시도 (`tryAcquireLeader`)**
    1. `SETNX` 방식(`setIfAbsent`)으로 리더 Key를 선점
4. **리더 획득 성공 시 로그 출력 / 실패 시 다른 서버가 리더임을 기록**
...
1. 애플리케이션 시작 시 @StreamConsumer 메서드 스캔
2. StreamConsumerRegistry에 핸들러 등록
3. StreamMessageListenerContainer에 스트림 구독 설정
4. 메시지 수신 시 적절한 타입으로 변환 후 메서드 호출
5. autoAck=true인 경우 자동 승인 처리
