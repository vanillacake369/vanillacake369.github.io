...
1. BIOS 다운로드
2. ROM 파일 준비 및 권한 설정
3. Libvirt XML 편집 / QEMU 명령어 수정
4. 이후 ROM 매핑이 성공했는지 dmesg 확인하여 admgpu 드라이버가 정상적으로 인식되었는지 확인한다.
...
NVIDIA는 `nvidia-device-plugin`을 사용하며, 리소스 이름으로 `nvidia.com/gpu`를 사용합니다. 최근에는 Helm을 통한 **GPU Operator** 설치를 권장하는 추세입니다.
...
[https://instinct.docs.amd.com/projects/k8s-device-plugin/en/latest/](https://instinct.docs.amd.com/projects/k8s-device-plugin/en/latest/)
