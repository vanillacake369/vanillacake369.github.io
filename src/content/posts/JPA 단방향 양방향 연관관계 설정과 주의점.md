...
1. 연관관계의 주인은 외래키가 있는 곳
2. 연관관계 주인 설정의 기준은 외래키 관리를 해야하는 엔티티

## 양방향

### 양방향 vs 단방향 결정 기준

1. 비즈니스 요구사항에 따라, 본인 테이블을 사용하여 다른 테이블을 역참조를 할 필요가 없다면
...
1. 토끼책 질문
2. 양방향 연관관계 편의메서드를 요구사항에 다른 요청에 따라 한 쪽 도메인에서 update,remove를 정의해주었다. 이렇게 하는 게 맞나??
3. Test 코드 시, yml 은 어떻게 보호하나??
4. 단방향 연관관계에서는 역참조를 할 수가 없다. 그럼 어떻게 상대방 엔티티를 찾아오는가??
5. 다른 도메인에 대한 검증 :: 서비스 주입 vs 레포지토리 주입
