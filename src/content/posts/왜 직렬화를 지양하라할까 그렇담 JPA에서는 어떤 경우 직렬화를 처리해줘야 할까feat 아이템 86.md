...
초기 버전에서 A형태의 직렬화를 제공하는 API를 구성하게 되었다 하자.

다음 API 배포 시, A` 형태로 클래스가 수정확장되게 되었다.

A 형태의 직렬화를 의존하던 이전 API 사용자들은 A`형태의 직렬화를 사용할 수가 없다.
( 왜냐하면 A형태의 직렬화는 A형태의 기본 생성자를 의존하는 것이나 마찬가지이기 때문이다)
**따라서 초기에 A형태 직렬화를 제공하기로 하였다면, 무조건 계속 A형태의 직렬화를 제공할 수 밖에 없다.**

- 버그와 보안구멍

역직렬화는 byte stream을 제공하게 된다.

즉, 해당 인스턴스에 대한 기본 생성자를 제공하게 된다.

이를 통해 **인스턴스를 조작할 수 있는 기회를 제공할 수 있다.**

- 버전 릴리스 별 직렬화를 테스팅 해야함

우리는 항상 버저닝을 하면서 의존성을 확장하게 된다.

초기에는 간단한 시스템이, 자라면 자랄수록 복잡한 요구사항으로 인해 의존성이 추가되게 때문이다.

직렬화는 유연한 설계를 방해한다.

이전 API 버전에서 제공한 직렬화 메서드가, 다음 API 버전에서도 동일하게 적용되어야 하기 때문이다.
...
The specification requires that fields of an entity may be Java primitives, Java serializable types, or user-defined serializable types.
...
In rare cases of complex entity classes, our reference must be serializable.
...
By creating DTOs, we decouple internal domain objects from external services. **If we want to expose our entities directly to the presentation layer without DTOs, then entities must be serializable**.
...
따라서 POJO 내부의 필드들은 모두 직렬화 가능해야만, 이를 key-value형태의 String으로 매핑시켜줄 수 있다.
...
Jackson을 통해 JSON으로 매핑시켜줄 때 아래와 같은 로직을 처리하게 된다.

따라서, 웬만하면 DTO를 사용하여 매핑시켜주도록 하자~!~!
