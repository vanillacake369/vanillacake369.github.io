...
> 하나의 특징으로 인해 아래 네 가지 이슈가 발현된다. 그 특징은 바로바로,,,
...
우리는 항상 버저닝을 하면서 의존성을 확장하게 된다. 초기에는 간단한 시스템이, 자라면 자랄수록 복잡한 요구사항으로 인해 의존성이 추가되게 때문이다. 
직렬화는 유연한 설계를 방해한다. 
...
상속용 클래스라면 
지정한 직렬화메서드가 **잘 돌아가는지** 확인해야한다. 재정의하게 된다면 재정의한 직렬화가 **다음 상속에 영향을 끼치지 않는지 **확인해야한다.
상속용 인터페이스라면 
...
> [Hibernate official documentation](https://hibernate.org/orm/documentation/), when describing mapping in Hibernate, notes that the** referenced field must be serializable when we use *****referencedColumnName *****from the **[***@JoinColumn ***](https://www.baeldung.com/jpa-join-column)annotation. Usually, this field is a primary key in another entity. In rare cases of complex entity classes, our reference must be serializable.
