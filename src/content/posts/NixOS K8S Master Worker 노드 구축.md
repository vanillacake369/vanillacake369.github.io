---
description: "NixOS 환경에서 kubeadm으로 K8S Master·Worker 노드를 구축하는 과정. 토큰·CNI·TLS 부트스트랩 원리까지 상세히 설명한다."
date: 2026-01-20
tags: [homelab, nix]
lang: ko
draft: false
series: { id: "NixOS Ecosystem", order: 7 }
---
...
Master 초기화 (`kubeadm init`) 2.

CNI (Flannel) 설치 3.

Join 토큰 생성 및 검증 준비 4.
...
노드 연결 및 등록 상태 확인 (`Ready` 여부) 2.

Kubelet 서비스 엔진 상태 분석 3.

클러스터 핵심 컴포넌트(System Pods) 생존 확인 4.
...
KUBELET_KUBEADM_ARGS="" 빈 문자열 에러 3. kubelet PATH에 mount가 없음 4. br_netfilter 커널 모듈 미로드 (MicroVM) 5.
