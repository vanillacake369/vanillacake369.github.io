...
홈랩에 Kubernetes 클러스터를 구축한다고 가정해보자.
물리 호스트 1대 위에 VM 3대를 올리고, 각 VM에 컨테이너 런타임, kubelet, CNI를 설정해야 한다.
Terraform이나 OpenTofu로 VM을 프로비저닝할 수 있겠지만, OS 내부 — 커널 모듈, sysctl 파라미터, systemd 서비스 — 까지는 이 도구들이 닿지 않는다[^1].

이때 NixOS를 도입하면 커널부터 부트로더까지 `.nix` 파일 하나로 선언할 수 있다.
그런데 이 구조에는 숨겨진 문제가 있다.
물리 호스트 1대, VM 3대만으로도 `.nix` 파일이 30개를 넘기고, 노드가 늘어날수록 파일 간의 설정 불일치 — 이른바 **코드 수준의 configuration drift** — 가 발생한다[^2].

NixOS가 보장하는 것은 "코드와 시스템의 일치"이지, "코드와 코드의 일관성"이 아니기 때문이다.
이 글은 그 복잡성을 통제하는 세 가지 설계 원칙 — **계약 기반 설계**, **합성 가능한 계층(Atom → Role → Node)**, **순수 함수 헬퍼** — 을 다룬다.

## Why NixOS? 🐧

Terraform, Pulumi, OpenTofu 같은 도구는 _클라우드 리소스의 선언적 관리_ 에 탁월하다.
