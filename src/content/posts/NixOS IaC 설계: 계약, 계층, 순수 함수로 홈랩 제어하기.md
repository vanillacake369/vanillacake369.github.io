---
description: "NixOS 홈랩에서 .nix 파일 30개 이상의 복잡성을 계약 기반 설계, Atom→Role→Node 계층, 순수 함수 헬퍼로 통제하는 방법을 다룬다."
date: 2026-05-08
...
홈랩에 Kubernetes 클러스터를 구축한다고 가정해보자. 물리 호스트 1대 위에 VM 3대를 올리고, 각 VM에 컨테이너 런타임, kubelet, CNI를 설정해야 한다. Terraform이나 OpenTofu로 VM을 프로비저닝할 수 있겠지만, OS 내부 — 커널 모듈, sysctl 파라미터, systemd 서비스 — 까지는 이 도구들이 닿지 않는다[^1].

이때 NixOS를 도입하면 커널부터 부트로더까지 `.nix` 파일 하나로 선언할 수 있다. 그런데 이 구조에는 숨겨진 문제가 있다. 물리 호스트 1대, VM 3대만으로도 `.nix` 파일이 30개를 넘기고, 노드가 늘어날수록 파일 간의 설정 불일치 — 이른바 **코드 수준의 configuration drift** — 가 발생한다[^2].

NixOS가 보장하는 것은 "코드와 시스템의 일치"이지, "코드와 코드의 일관성"이 아니기 때문이다. 이 글은 그 복잡성을 통제하는 세 가지 설계 원칙 — **계약 기반 설계**, **합성 가능한 계층(Atom → Role → Node)**, **순수 함수 헬퍼** — 을 다룬다.

## Why NixOS? 🐧

Terraform, Pulumi, OpenTofu 같은 도구는 _클라우드 리소스의 선언적 관리_ 에 탁월하다. 그러나 kernel 수준의 IaC 제어는 불가능하다. 이는 IaC 의 abstraction layer 때문이다. 
...
다만 이렇게 완벽해 보이는 NixOS 에도 단점이 숨어 있다. 하나는 언어의 복잡성이고, 하나는 파일 구조에 숨어 있다.

## NixOS 의 양면성: 커널 제어만큼 다양한 IaC 코드 ⚙️

NixOS 가 커널 수준까지 제어한다는 것은, 그만큼 IaC 코드 파일이 다양해지고 많아진다는 것을 의미한다.
그렇다고 디렉토리나 파일을 역할 별로 쪼개자니 굉장히 복잡해지고, 이로 인해 코드 복잡성이 올라간다.
그냥 본인이 관리할 홈랩인데 코드 복잡성이 무슨 상관인가 싶겠지만 매우. 굉장히. 아주아주. 중요하다.
...
디렉토리 구조는? 함수형이니 OOP 는 못 쓰나? 재사용은?
추상화에 따른 세부 구현은? 정적값 처리는?

이 글은 바로 이 질문들에서 출발한다.
NixOS 의 선언적 코드가 폭발적으로 늘어날 때, _어떤 구조로 통제할 수 있는가_.
...
패키지 선언, 서비스 활성화, 방화벽 포트 개방, 필요하다면 커널 모듈 로드까지 — 전부 `.nix` 파일에 적혀 있어야 한다.
...
물리 호스트 1대, VM 3대. _작은_ 홈랩이다.

그런데도 `.nix` 파일만 30개가 넘는다.

노드가 늘어날수록, 역할이 다양해질수록 이 숫자는 가속도가 붙는다.

VM 을 두 대만 더 추가해도 `.nix` 파일은 40개를 넘기고, 역할이 하나 더 생기면 그 역할에 딸린 파일들이 줄줄이 따라온다.
...
진짜 문제는 _파일이 많아졌을 때 일관성을 잃는 것_ 이다.

### 설정이 흩어지면 벌어지는 일: Configuration Drift 의 귀환

NixOS 를 도입한 이유 중 하나는 Ansible 이나 Chef 가 시달리던 _configuration drift_ 를 없애기 위함이었다.
...
코드에 적힌 대로 시스템이 구성되니까, _코드와 시스템 사이의 drift_ 는 원리적으로 불가능하다.

그런데 `.nix` 파일들이 체계 없이 늘어나면, drift 가 _OS 수준_ 이 아니라 _코드 수준_ 에서 되살아난다.
...
NixOS 가 보장하는 것은 _"코드와 시스템의 일치"_ 이지, _"코드와 코드의 일관성"_ 이 아니다.
...
NixOS 에서 이 "필수 항목" 역할을 하는 도구가 `lib.mkOption` 이다[^3]. `mkOption` 으로 선언된 옵션은 어딘가에서 반드시 값이 채워져야 한다.
...
> _"이 홈랩의 모든 노드는 IP, 호스트 타입, 역할을 반드시 선언해야 한다. _
...
VM 은 가상 네트워크 인터페이스를 가지므로 MAC 주소가 반드시 있어야 한다. default = null 때문에 타입 검사는 통과하지만, 실제로는 있어야 하는 값이다.

이런 조건부 규칙은 타입 시스템만으로 표현할 수 없다.

### 빌드 타임에 터지는 안전망: assertion 이 잡아내는 것들

mkOption 이 "이 칸은 필수입니다"라는 표시라면, assertion 은 "이 칸과 저 칸 사이에 이런 관계가 있어야 합니다"라는 규칙이다[^4].

다시 입사 지원서로 돌아가보자. "희망 직무"에 "개발자"를 적었다면 "포트폴리오" 항목도 반드시 채워야 한다는 규칙이 있다고 하자. "디자이너"를 적었다면 포트폴리오는 선택이다.
...
계약이 "무엇을 반드시 채워야 하는가" 를 강제한다면, 이번 절은 "공통된 것을 어떻게 한 번만 적을 것인가" 에 대한 이야기다.
...
이 프로젝트에서 모든 노드가 공유하는 기반 역할인 common.nix 부터 보자.
...
그리고 마스터 노드에 필요한 방화벽 포트를 연다. common.nix 에서 이미 SSH 포트(22)가 열려 있으므로, 여기서는 K8s API 서버(6443), etcd(2379, 2380), kubelet(10250) 등 마스터 고유 포트만 추가하면 된다.
...
두 파일의 구조가 동일하다. `node = { ... }` 블록에서 계약을 이행하고, imports 에서 역할을 선택한다.
...
그러나 계층 구조를 아무리 잘 짜도, 노드를 추가할 때마다 flake.nix 에 손으로 등록해야 한다면 그것 역시 실수의 온상이다.
...
parentHost 를 삭제하면? parentHost 는 default = null 이므로 타입 검사는 통과하지만, `"vm" -> parentHost != null assertion` 에서 막힌다.
...
이전 글에서 다룬 빌드 파이프라인의 4단계 — Eval → Drv → Realise → Activate — 중 앞 두 단계만 실행하는 셈이다[^2]. "이 설정으로 빌드하면 무엇이 만들어지는가"까지만 확인하고, 실제로 만들거나 배포하지는 않는다.
...
[^1]: Terraform Provider vs NixOS — abstraction layer 차이. <https://nixos.wiki/wiki/Comparison_with_Terraform>

[^2]: Configuration Drift 개념. <https://www.hashicorp.com/resources/what-is-drift-detection>

[^3]: NixOS Module System — mkOption, assertion. <https://nix.dev/tutorials/module-system/>

[^4]: NixOS Option Types 공식 문서. <https://nixos.org/manual/nixos/unstable/#sec-option-types>

[^5]: Colmena — NixOS 멀티노드 배포 도구. <https://colmena.cli.rs/>

[^6]: Disko — 선언적 디스크 파티셔닝. <https://github.com/nix-community/disko>

[^7]: SOPS-Nix — 시크릿 관리. <https://github.com/Mic92/sops-nix>

[^8]: Dolstra, E. (2006). The Purely Functional Software Deployment Model. <https://edolstra.github.io/pubs/phd-thesis.pdf>

[^9]: 프로젝트 소스 코드. <https://github.com/vanillacake369/tonys-homelab/tree/572a89c>

[^10]: mk-libvirt.nix — VM 도메인 생성 헬퍼. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/lib/mk-libvirt.nix>
