---
description: echoTony 커스텀 명령어를 TDD 방식으로 추가하며, commands.def 등록부터 c->cmd->proc(c) 실행까지 명령어 처리 파이프라인을 추적한다.
tags:
    - tools
    - infra
lang: ko
...
`function` 필드의 값 `"echoTonyCommand"` 가 `commands.def` 의 `MAKE_CMD` 매크로에서 함수 포인터로 등록된다. `arity: 2`는 명령어 이름을 포함해 인자가 정확히 2개라는 의미다(`ECHOTONY message`). 이후 코드 생성 스크립트를 실행하여 `commands.def`를 갱신한다.
