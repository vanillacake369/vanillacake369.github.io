...
| `hasexpire` | 1 비트      | 만료 시간이 설정되어 있는지 여부                               |
| `hasembkey` | 1 비트      | key 가 robj 에 embedded 되어 있는지 여부                       |
| `hasembval` | 1 비트      | value 가 robj 에 embedded 되어 있는지 여부                     |
| `refcount`  | 29 비트     | 공유 객체의 참조 카운트 (예: 정수 0–9999 는 공유 풀)           |
| `val_ptr`   | 64 비트     | 실제 자료구조를 가리키는 포인터. `objectGetVal(obj)` 로 접근   |
...
void hashTypeConvertListpack(robj *o, int enc) {
    if (enc == OBJ_ENCODING_HASHTABLE) {
        ...
        while (p != NULL) {
            // listpack 에서 field 와 value 를 차례로 읽어 sds 로 변환
            unsigned int slen;
            unsigned char *vstr = lpGetValue(p, &slen, NULL);
            sds field = sdsnewlen(vstr, slen);
            p = lpNext(o->ptr, p);
            vstr = lpGetValue(p, &slen, NULL);
            sds value = sdsnewlen(vstr, slen);
            p = lpNext(o->ptr, p);
...
int streamAppendItem(stream *s, robj **argv, int numfields,
    ...
    raxIterator ri;
    raxStart(&ri, s->rax);
    raxSeek(&ri, "$", NULL, 0);  // 끝 위치로 이동
    raxPrev(&ri);               // 마지막 키 (가장 큰 ID 의 listpack 노드)
...
> **무작위 비트 시퀀스에서 연속되는 0 의 최대 개수가 K 라면, 본 시퀀스의 수는 약 2^K 일 가능성이 높다.**

각 멤버를 hash 하여 64 비트로 만든 뒤, 하위 14 비트로 register 인덱스 (0 ~ 16383) 를 결정하고
나머지 상위 50 비트에서 trailing zero 수 (최하위 비트부터 연속되는 0 의 개수) 를 계산해 그 register 에 기록한다. Valkey 구현은 `__builtin_ctzll()` 로 trailing zero 를 세며, 이는 원래 HLL 논문의 "leftmost 1-bit position" 과 동치이다.

전체 카디널리티는 모든 register 의 trailing zero 값들의 조화 평균을 기반으로 추정된다.
...
```c
// src/hyperloglog.c (단순화) — hash 의 trailing zero 를 register 에 max 기록
