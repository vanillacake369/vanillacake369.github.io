...
핵심을 하나만 꼽자면, 읽기와 쓰기는 완전히 분리된 이벤트 경로를 따르며, 그 사이를 `clients_pending_write` 큐가 잇는다는 점이다. [이전 글](/posts/valkey-커스텀-명령어-구현하기)에서 `addReplyBulkSds()`가 클라이언트 버퍼에 응답을 쌓는 것까지 확인했는데, 그 버퍼가 실제로 소켓에 쓰이는 경로가 바로 이 write 경로다.
