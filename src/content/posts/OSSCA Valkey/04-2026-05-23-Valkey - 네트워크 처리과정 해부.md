...
[이전 글](../03-2026-05-23-valkey---커스텀-명령어-구현하기)에서 `c->cmd->proc(c)`가 호출되면 커스텀 명령어가 실행되는 것을 확인했다. 그런데 클라이언트가 보낸 바이트가 어떻게 이 함수 호출까지 도달하는가?

Valkey는 싱글 스레드 이벤트 루프 기반으로 수만 개의 동시 연결을 처리한다. 스레드 하나가 수만 fd를 관리할 수 있는 이유는 커널의 I/O multiplexing — Linux에서는 `epoll` — 을 활용하기 때문이다. 이 글에서는 커널에서 시작하여 이벤트 루프, 콜백, 응답까지 도달하는 전체 네트워크 경로를 소스 코드 수준에서 추적한다.
...
`readToQueryBuf()`[^13]는 소켓에서 읽은 바이트를 `c->querybuf`에 누적한다. 그 후 `processInputBuffer()`가 RESP 프로토콜을 파싱하고, 최종적으로 `c->cmd->proc(c)`를 호출한다. [이전 글](../03-2026-05-23-valkey---커스텀-명령어-구현하기)에서 구현한 `echoTonyCommand()`가 바로 이 지점에서 실행된다.
...
Valkey의 I/O 경로는 세 개의 독립적인 이벤트로 구성된다.
...
`ae` 레이어가 epoll/kqueue/select 차이를 숨기고, `ConnectionType` vtable이 TCP/TLS/Unix 차이를 숨긴다. 두 계층의 추상화 덕분에 `networking.c`는 OS도 transport도 알지 못한 채 `connRead()`, `connWrite()`만 호출한다.

핵심을 하나만 꼽자면, 읽기와 쓰기는 완전히 분리된 이벤트 경로를 따르며, 그 사이를 `clients_pending_write` 큐가 잇는다는 점이다. [이전 글](../03-2026-05-23-valkey---커스텀-명령어-구현하기)에서 `addReplyBulkSds()`가 클라이언트 버퍼에 응답을 쌓는 것까지 확인했는데, 그 버퍼가 실제로 소켓에 쓰이는 경로가 바로 이 write 경로다.
