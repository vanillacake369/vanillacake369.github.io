...
1. hostnamectl
    ...
    hostnamectl 만 호출하면 현재 접속한 host 에 대한 내용을 볼 수있다.

    ```bash
    bob@ubuntu-host ~ ➜  hostnamectl

     Static hostname: ubuntu-host
           Icon name: computer-container
             Chassis: container
          Machine ID: 56e82f441b024b54abbd0…
             Boot ID: 3e1524e48e3b41ce985f1…
      Virtualization: container-other
    Operating System: Ubuntu 22.04.5 LTS
              Kernel: Linux 6.8.0-1047-gcp
        Architecture: x86-64
    ```

    아래 명령어를 통해 hostname 을 수정

    ```bash
    sudo hostnamectl set-hostname <new-hostname>
    ```

    hostnamectl 은 D-Bus 메시지를 systemd-shotnamed 데몬에게 보내는 클라이언트이며

    해당 명령어는 아래와 같은 작업을 처리한다

    ```bash
    hostnamectl set-hostname new-hostname
    │
    ├→ /etc/hostname 파일 수정          (디스크 영구 반영)
    ├→ sethostname() syscall 호출       (커널 버퍼 즉시 반영)
    └→ /proc/sys/kernel/hostname 변경  (syscall 결과가 노출됨)
    ```

    다만 /etc/hosts 에 대해서는 수정하지 않으므로 해당 부분은 수정 변경이 필요하다

2. /etc/hosts 수정

    /etc/hosts 는 DNS 조회 전에 먼저 참조하는 IP 매핑 테이블이다.

    여기에는 본인 ip 에 대한 loopback 에 대해 hostname 과 매핑되어있는 값이 저장되어있다.

    따라서 sudo vi /etc/hosts 를 통해 hostname 을 수정해줘야한다.

    ```bash

    127.0.1.1   ubuntu-host   ← 이 부분을 new-hostname으로 수정

    ```

    > 💡
    >
    > /etc/hosts 는 ${이름}에 대해 IP 로 매핑해주는 내부 DNS 테이블이다.
    >
    > 여기에는 등록한 사용자와 호스트, 또 뭐가 정의되나 ??
    >
    > ```bash
    > 127.0.0.1 localhost   # loopback 주소 -> 자기자신 매핑
    > ::1 localhost         # 위와 동일, 그러나 IPv6 버전. 127.0.0.1 이 IPv4 loopback 이면, ::1 은 IPv6 loopback
    > 127.0.0.2 ubuntu-host # loopback 주소 -> hostname 매핑
    > ```
    >
    > /etc/hosts 를 사용한 network flow 는 다음과 같다.
    >
    > 1. IP 가 필요한 프로세스가 IP 를 질의
    > 2. OS 는 `/etc/nsswitch.conf`를 열어 "조회 순서가 뭔지" 확인
    >
    >     ```bash
    >     ~                                                                                                                                                                                         limjihoon@homelab
    ...
    >     ```
    >
    >     hosts 를 확인할 때 `hosts:` 로 되어있는 부분을 확인한다.
    >
    >     위 예시에서 `hosts: mymachines resolve [!UNAVAIL=return] files myhostname dns` 라고 되어있는데 다음과 같은 순서로 확인한다.
    >     1. `mymachines` systemd-machined 가 관리하는 컨테이너/VM hostname
    >     2. `resolve [!UNAVAIL=return]` → 데몬이 살아있으면 여기서 종료 (3~5 스킵)
    >
    >         ↓ 데몬이 죽어있을 때만 아래로 fallback
    >
    >     3. **`files` -> /etc/hosts 정적 테이블 확인**
    >     4. `myhostname` -> systemd-hostnamed 를 통해 hostname 질의
    >     5. `dns` -> /etc/resolv.conf 의 nameserver 를 확인하여 DNS 질의
    >
    > 즉, `/etc/nsswitch.conf` 에 정의되어있는 도메인 Resolution 처리 순서에 따라 `/etc/hosts` 에 도달하게되며 여기서 정의된 hostname 에 대한 ip loopback 을 처리하게되는 것이다.
...
- 기호 방식
  \_ \_ \_ 에 대해 <who><op><perm> 으로 분리하여 지정하는 방식
    - who 에는 u(user/owner), g(group), o(others), a(all) 가 온다
    - op 에는+(추가), -(제거), =(지정), 빈칸(void) 가 온다
    - perm 에는 r, w, x 이 온다
    ```bash
    chmod u+x  file.txt   # owner 에게 실행 권한 추가
    chmod g-w  file.txt   # group 의 쓰기 권한 제거
    chmod o=r  file.txt   # others 를 읽기만 가능하게 지정
    chmod a+x  file.txt   # 모두에게 실행 권한 추가
    chmod u+   file.txt   # group 에 모든 권한 제거
    chmod g+   file.txt   # group 에 모든 권한 제거
    chmod a+   file.txt   # 모두에게 모든 권한 제거
    ```
- 8진수 방식
  \_ \_ \_ 에 대해서 owner / group / others 로 분리하여
  각각에 대한 8진수값을 지정한다
  r,w,x,-(void) 에 대해 4,2,1,0 으로 지정된다
  예시를 들면 아래와 같다.
    ...
    chmod 777 file.txt   # 모두 rwx (보안상 위험)
    ```
...
>
> tar 명령어는 여러 파일을 아카이브로 변환하여 tar 파일을 생성한다. 또한 tar 아카이브를 추출하고, 아카이브에 포함된 파일 목록을 표시하며, 기존 아카이브에 파일을 추가하는 등 다양한 작업을 수행한다.
>
> tar는 원래 파일을 magnetic tape에 저장하기 위한 아카이브를 생성하도록 설계되었기 때문에 “Tape ARchive”라는 이름이 붙었다.
...
1. tar 파일 생성
    ...
    tar -cvf archive.tar file1 file2 file3 # -v 옵션으로 진행상황 바로 보기
    ```
...
2. gz archive 압축 파일 생성
    ...
    tar -czvf archive.tar.gz file1 file2 # -v 옵션으로 진행상황 바로 보기
    ```
...
3. tar 파일 해제
    ...
    tar -xvf archive.tar # -v 옵션으로 진행상황 바로 보기
    ```

4. tar 파일해제 없이 압축된 내용 보기

    ```bash
    tar -tvf archive.tar # -v 옵션으로 세부내용(크기,소유자,,,) 같이 보기
    ```
...
1. 리다이렉션 기호 (`>`, `2>`, `&>`)
    - **`>` (또는 `1>`)**: 정상 출력만 파일로 보냄. (기존 내용 삭제 후 새로 작성)
    - **`>>`**: 정상 출력만 파일 끝에 추가함.
    - **`2>`**: 에러 메시지만 따로 파일로 보냄.
    - **`&>`**: 정상 출력과 에러를 **모두** 한 파일에 담음.
2. 특수 장치와 기호 (`/dev/null`, `|`)
    - **`/dev/null` : 특수한 null virtual device file 이다. 여기로 데이터를 보내면 바로 값이 다 삭제된다.**
        ```bash
        /home/bob/script.sh 2> /dev/null
        ```
    - **`|` (파이프) :** 앞 명령어의 출력을 뒤 명령어의 입력으로 바로 넘겨준다.
        ```bash
        ls -l | grep ".txt" # ls -l 의 출력값에 대해서 ".txt" 를 grep
        ```

- 스크립트 실행결과에 대해 정상결과와 에러결과를 각각의 파일로 저장
  ...
  Validate "/home/bob/output_stdout.txt" file.
    ...
    ```
- 스크립트 실행결과에 대해 정상결과, 에러결과 모두를 하나의 파일로 저장
...
## **SSL Certificates 🔒 ** ⚠️정리가 필요함 ⚠️ \*\*\*\*

> 💡
>
> SSL/TLS 에 대한 사전지식이 필요하다
>
> 모르겠다면 [About TLS](https://www.notion.so/About-TLS-33219c39029080fc9f2bccd1fb7dbe29?pvs=21) 를 읽고오자
...
> 💡
>
> `man openssl` vs `man openssl req`
>
> openssl 은 하나의 man 이 아니라 각 하위 명령어 별로 man 을x6 가지게된다.
>
> 따라서 man openssl 만 명령하면 openssl 이 제공하는 하위 명령어, 암호화 알고리즘, message digest 명령어의 목록을 보여주게된다.
>
> 만약 우리가 인증서를 처리하고자하는 openssl req 에 대해서는 보고자한다면
>
> man openssl req 을 치면 된다.
>
> 여기에 모든 EXAMPLE 이 보여진다.

# 03. Operations Deployment

![Pasted image 20260408172923.png](attachment:b91d69d9-b24a-40c2-a5d6-4629d5afa8ae:Pasted_image_20260408172923.png)
...
![Pasted image 20260408172524.png](attachment:372e8ddc-1793-4076-98fe-0dd3b31045a0:Pasted_image_20260408172524.png)

> 💡 목표
>
> 리눅스 네트워크를 제대로 익히려면 “명령어”보다 먼저 **처리 순서와 경계(레이어)** 가 머릿속에 있어야 한다.
> 이 섹션은 패킷이 애플리케이션에서 NIC까지 내려가는 흐름과, 그 과정에서 **DNS / 라우팅 / 방화벽**이 어떤 위치에서 어떤 역할을 하는지 구조적으로 소개한다.
...
> 💡 핵심 관점
>
> - Bridge는 “여러 포트를 연결”한다.
> - Bond는 “여러 링크를 하나처럼 보이게” 한다.
> - 둘 다 최종적으로는 `ip link`에 보이는 **인터페이스**이며, IP는 보통 “하위 포트”가 아니라 “상위 인터페이스(br0/bond0)”에 올린다.
...
> ⚠️ 주의
>
> 브리지에 붙이는 물리 NIC에는 보통 IP를 직접 올리지 않는다(올리면 라우팅/ARP가 꼬이기 쉽다).
...
> 💡 참고
>
> - Bridge 관련: `bridge`(iproute2) 명령이 실무에서 유용하다.
> - Bond 관련: `/proc/net/bonding/bond0`는 상태 확인의 “정답지”에 가깝다.
...
> 💡 핵심
>
> - `ufw`는 “내가 원하는 정책”을 선언적으로 적고, 실제로는 커널(netfilter)에 룰을 생성한다.
> - 단순 서버라면 대부분은 **INPUT**(인바운드)만 잘 관리해도 충분한 경우가 많다.
...
> ⚠️ 주의
>
> SSH 허용 없이 `ufw enable`부터 하면, 원격 세션이 끊길 수 있다.
...
> 💡 참고
>
> UFW는 “필터링”을 담당한다. 라우팅/포워딩(NAT 포함)은 다음 섹션에서 netfilter와 함께 이어서 정리한다.
...
> 💡 정리
>
> “chain을 수정한다”는 말은, 결국 “특정 table의 특정 chain에 rule을 추가/삭제한다”는 뜻이다.
> 그리고 `DNAT`, `SNAT`, `MASQUERADE`는 chain 이름이 아니라 **타깃(액션)**이다.
...
> ⚠️ 주의
>
> 이 규칙이 동작하려면 **리턴 패킷이 다시 이 머신을 경유**해야 한다.
> 즉, `192.168.0.50`의 기본 게이트웨이가 이 NAT 머신이거나, 그렇지 않다면 SNAT이 필요해질 수 있다.
...
>
> - DNAT은 보통 PREROUTING: “어디로 보낼지(라우팅) 결정하기 전에 목적지를 바꿔야” 한다.
> - SNAT/MASQ는 보통 POSTROUTING: “어느 인터페이스로 나갈지 결정된 뒤에 출발지를 바꿔야” 한다.
...
> 💡 DNS와의 관계(오해 포인트)
>
> - Reverse proxy는 “DNS Query를 대신 처리하는 서버”가 아니다.
> - 클라이언트는 **reverse proxy의 도메인**을 DNS로 해석해서 프록시로 접속한다.
> - 프록시가 upstream을 **hostname으로** 지정했다면, 프록시도 DNS를 통해 upstream을 해석할 수는 있다(구성/캐시/갱신 방식은 별도).
...
> 💡 `proxy_params`가 하는 일
>
> `/etc/nginx/proxy_params`에는 보통 upstream이 클라이언트 정보를 알 수 있도록 아래 같은 헤더가 들어있다.
>
> - `Host`
> - `X-Real-IP`
> - `X-Forwarded-For`
...
>
> `location /images/ { proxy_pass http://img_backend; }`처럼 쓸 때,
> 뒤에 `/`가 붙느냐에 따라 upstream으로 넘어갈 URI가 달라질 수 있다. (헷갈리면 “항상 한 패턴으로 고정”하고 `nginx -T`로 실제 설정을 확인한다.)
...
> 💡 참고
>
> - `timedatectl set-ntp true`는 “이 시스템에서 NTP 동기화를 켠다”는 의미다.
> - 실제로 어떤 데몬이 동작하는지는 배포판/설치 상태에 따라 다를 수 있다.
...
> ⚠️ 주의
>
> 시간이 심하게 틀어진 시스템은 step(급격한 시간 이동)이 발생할 수 있고, 이는 일부 서비스에 영향을 줄 수 있다.
...
>
> 운영에서는 “메인 파일을 크게 건드리지 않고”, 작은 단위의 설정을 별도 파일로 쌓는 편이 관리/리뷰/롤백에 유리하다.
> 다만 적용 순서/Include 정책은 배포판의 기본 `sshd_config`에 따라 달라질 수 있으니, 실제 시스템에서는 파일 안의 `Include` 라인을 먼저 확인한다.
...
>
> - `known_hosts`(클라이언트): “내가 접속하는 서버가 맞는가?”
> - `authorized_keys`(서버): “이 사용자가 맞는가?”
...
> ⚠️ 주의
>
> 원격 서버에서 `PasswordAuthentication no`로 바꾸기 전에, 키 기반 로그인(다른 세션 포함)이 이미 되는지 확인한다.
...
> 💡 `ListenAddress`에 대해
>
> `ListenAddress`는 “서버가 바인딩할 로컬 주소/인터페이스를 제한”하는 옵션이다.
> “클라이언트 IP 화이트리스트”는 보통 방화벽(UFW) 또는 `Match Address` 같은 메커니즘으로 처리한다.
...
![image.png](attachment:ef2a3531-c87d-4319-90bc-4a2172450a13:image.png)
...
[SSH 란? (feat. Nixos SSH 활성화)](https://www.notion.so/SSH-feat-Nixos-SSH-20f19c3902908018b512fec4e62ecce1?pvs=21)
...
![image.png](attachment:1c47ce04-56a7-47f9-a437-24143116cb5e:image.png)
...
https://www.linux-tips-and-tricks.de/en/general/282-was-kann-lvm-und-warum-sollte-man-lvm-benutzen-what-can-lvm-do-form-me-and-why-should-i-use-lvm-2
...
![image.png](attachment:5f8ff7cd-156c-4c93-8b2b-acff18812b7d:image.png)

![image.png](attachment:b82af3a6-2de5-451e-b07d-01d5a5e5ff8c:image.png)
...
>
> 1. LVM 패키지 설치
> 2. 디바이스 확인 (`lvmdiskscan`, `lsblk` 등)
> 3. PV 초기화 (`pvcreate`)
> 4. VG 생성 (`vgcreate`)
> 5. LV 생성 (`lvcreate`)
> 6. LV에 파일시스템 생성 및 마운트
> 7. 필요 시:
>     - VG 확장/축소 (`vgextend` / `vgreduce`)
>     - LV 리사이징 (`lvresize`)
> 8. 제거 시:
>     - LV 제거
>     - PV 제거 (`vgreduce` → `pvremove`)

> **LV가 특정 마운트를 통해 VG를 가리키는 구조**
>
> - V는 `/dev/<VG 이름>/<LV 이름>` 경로에 디바이스로 노출된다
> - LV 를 마운트하여 사용하고자 한다면 다음과 같이 마운트한다.
>     ```bash
>     sudo mkfs.xfs /dev/my_volume/partiona1
>     echo "/dev/my_volume/partiona1 /data xfs defaults 0 0" >> /etc/fstab
>     mount /data
>     ```
> - 위와 같이 처리되면 다음과 같이 연결되어 처리된다.
>   `/data` mount
>   → `/dev/my_volume/partiona1` LV
>   → `my_volume` VG
>   → VG 에 매핑된 `/dev/sdc` , `/dev/sdd` PV 내의 PE

> 제거 축소 순서
>
> 제거 시에는 데이터 손실을 막기 위해 “역순대로” 처리한다
>
> \*\*만약 역순대로 안 했으면 시래하거나 데이터/파일시스템 파손될 수 있다
>
> \*\*웬만하면 lvm 이 막아주지만, 강제옵션을 쓰거나 잘못진행하면 가능하다
>
> 1. LV 제거 `umount /data && lvremove /dev/my_volume/data_lv`
> 2. VG 에서 PV 제거 `vgreduce my_volume /dev/sde`
> 3. VG 축소/제거 `vgremove my_volume`
> 4. PV 제거 `pvremove /dev/sde`
