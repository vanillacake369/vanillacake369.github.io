---
description: "VLAN의 개념과 동작 원리를 정리하고, Linux Bridge를 활용한 VLAN 구축 방법과 트러블슈팅 명령어를 소개한다."
tags: [network]
lang: ko
draft: false
---
...
VLAN 을 설정해두면 노출된 특정 VM 이 공격당해도 다른 VM 들이 노출되지 않게한다. 그렇다면 VLAN 은 무엇일까? 또 VLAN 동작원리는 어떻게 되는가? 차근차근 알아가보면서 어떻게 구축할 수 있는지 실습까지 해보고자 한다.
...
VLAN으로 네트워크를 쪼개면 서로 통신이 불가능하다. 이때 통신이 필요하면 **L3(IP 계층)** 단계에서 길을 찾아줘야 한다.
...
- **왜 하는가?** IP 주소는 알지만 상대방의 MAC 주소를 모르면 통신이 안 된다.
- **의미:** `ip neigh`에 상대방 MAC이 `REACHABLE`로 떠 있다면, 최소한 **L2(브리지/VLAN) 연결은 정상**이라는 뜻이다.

만약 `INCOMPLETE`라면 VLAN 설정이나 케이블(Veth) 연결 문제일 확률이 높다.
...
- **의미:** 브리지 포트(`vmbr0`)에 어떤 VLAN ID가 허용(Allow)되어 있는지 확인한다.
...
가장 권장되는 현대적인 방식으로, 하나의 브릿지가 여러 VLAN을 동시에 처리하도록 설정한다. 예전 브릿지는 들어오는 모든 패킷을 단순히 전달만 했지만, 현재의 Linux Bridge는 패킷의 802.1Q 태그를 읽어서 "이 패킷은 10번 포트로만 보내야지"라고 결정하는 **L2 스위치**다.

[^1]: IEEE 802.1Q VLAN 표준 <https://ascentoptics.com/blog/ko/understanding-vlan-what-is-a-vlan-and-how-does-it-work/>

[^2]: Linux Bridge VLAN Filtering 공식 문서 <https://www.kernel.org/doc/html/latest/networking/bridge.html>

[^3]: systemd-networkd VLAN 설정 레퍼런스 <https://www.freedesktop.org/software/systemd/man/systemd.network.html>

[^4]: Proxmox VE – VLAN 설정 가이드 <https://pve.proxmox.com/wiki/Network_Configuration#_vlan_802_1q>

[^5]: nftables 공식 문서 – 패킷 필터링 <https://wiki.nftables.org/wiki-nftables/index.php/Main_Page>
