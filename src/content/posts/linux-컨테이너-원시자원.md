---
description: "컨테이너는 VM이 아니다. Linux 커널이 제공하는 namespace, cgroup, OverlayFS, veth 네 원시 자원의 조합이다. 이 편에서는 각 원시 자원의 커널 내부 구조와 K8s 매핑을 해부하고, 수작업으로 컨테이너를 직접 만들어 검증한다."
date: 2026-05-01
...
K8s 를 운영하다 보면 한 가지 가정이 굳어진다.

컨테이너는 잘 캡슐화된 단위이므로, 문제가 생기면 재시작하면 된다는 것이다.

실제로 대부분의 일시적 장애는 그렇게 해결된다.

그러나 K8s 는 바로 이 가정 위에서 동작하도록 설계되어 있다.

kubelet 은 Pod 스펙에 선언된 자원 제한을 커널 수준 메커니즘으로 집행하고, 네트워킹은 커널의 패킷 처리 hook 위에 구성되며, 파일시스템은 커널의 Copy-on-Write 레이어 위에서 동작한다.

그 덕분에 재시작이 가능한 것이고, 재시작으로 해결되지 않는 순간에는 그 메커니즘 자체를 들여다봐야 한다.

실제 장애에서 이 한계가 드러나는 세 장면이 있다.
...
세 장면 모두 재시작으로는 해결되지 않는다.

근본 원인이 커널 원시 자원의 동작 방식에 있기 때문이다.

이 반복되는 장면의 실제 이름은 **cgroup OOM**, **CFS throttling**, **br_netfilter 미로드**[^L1]다.

이 편에서는 커널의 네 원시 자원 — namespace, cgroup, OverlayFS, veth — 의 내부 구조를 해부하고, 각 장면의 원인을 추측이 아닌 추적으로 풀 수 있는 관찰 방법을 다룬다.
...
커널 구조를 알아야 각 원시 자원이 커널의 어디에 위치하는지 파악할 수 있고, 관찰 도구(`/proc`, `/sys`)를 알아야 이후 실습에서 각 자원의 상태를 직접 확인할 수 있기 때문이다.
...
`/proc` 최상위 구조를 확인하면 숫자 디렉토리(PID)와 커널 전역 정보 파일이 함께 보인다.
...
숫자 디렉토리는 각 프로세스의 PID 이며, `/proc/<pid>/` 안에는 해당 프로세스의 주요 정보가 파일로 노출된다.
...
`/sys` 최상위 구조를 보면 커널이 노출하는 객체 범주를 한눈에 파악할 수 있다.
...
- `/sys/class/net/` — 각 인터페이스 디렉토리 안에 MTU, MAC 주소, 통계가 파일로 노출된다.

CNI 플러그인이 만든 가상 인터페이스 (`cali*`, `cilium*`, `flannel.1` 등) 를 확인할 수 있다.

kubelet 이 `/proc` 와 `/sys` 를 어떻게 나눠 사용하는지를 구조도로 보면 역할 분리가 명확해진다.
...
현재 프로세스의 PID, cgroup 경로, namespace inode, cgroup controller, 네트워크 인터페이스 MTU 를 순서대로 확인한다.
...
`/proc/$$/ns/` 에서 8개의 심링크가 보인다면, 각 심링크 뒤의 숫자 (예: `net:[4026531992]`) 가 namespace 의 inode 번호이며, 이 번호로 두 프로세스가 같은 namespace 에 속하는지 판별한다.

여기까지 `/proc` 와 `/sys` 로 커널 상태를 읽는 방법을 확인했다.
...
`nsproxy` 구조체의 실제 필드 배치를 단순화하면 다음과 같다.
...
task A 와 B 가 같은 namespace 를 공유하고, task C 가 일부 namespace 를 달리 쓰는 구조를 다이어그램으로 보면 reference count 의 의미가 명확해진다.
...
`clone()` 에 여러 `CLONE_NEW*` 플래그를 동시에 전달하면 단 한 번의 시스템 콜로 격리된 프로세스가 생성된다.
...
이 한 호출로 새 프로세스는 자기만의 PID 공간, 마운트 네임스페이스, 네트워크 스택, hostname, IPC 공간, cgroup view 를 가지게 된다.
...
`readlink` 로 두 프로세스의 namespace inode 를 읽으면 격리 여부를 즉시 판별할 수 있다.
...
`pause.c` 의 본체는 시그널 핸들러를 등록한 뒤 무한 루프로 `pause(2)` 를 호출하는 것이 전부다.
...
kubelet 이 CRI 를 통해 Pod 를 만드는 순서를 보면 pause container 의 역할이 구조적으로 드러난다.
...
PID namespace 격리와 inode 비교를 순서대로 수행한다.
...
새 namespace 안에서 PID 가 1 인 것이 확인된다.

이것이 "프로세스 그룹마다 서로 다른 PID 공간을 갖게 한다" 는 원리의 물리적 실체이며, 컨테이너 안에서 `kill 1` 이 컨테이너를 종료시키는 이유다.
...
셸 A 와 B 는 같은 inode 를 공유하고, 셸 C 는 새 namespace 로 인해 다른 inode 를 가진다.

`unshare --net` 이 실제로 새 net namespace 를 생성했음이 숫자로 확인된다.

여기까지 namespace 로 시야를 분리하는 메커니즘을 확인했다.

그런데 namespace 만으로는 한 가지 문제가 해결되지 않는다.
...
v1 구조에서는 CPU, 메모리, 블록 I/O 각각에 독립된 디렉토리 트리가 존재했다.
...
v2 의 unified hierarchy 아래 K8s 가 만드는 경로 구조는 다음과 같다.
...
| `memory.high` | soft limit. 초과 시 회수 압력 발생 |
...
K8s 가 쓰는 실제 cgroup 경로에서 `cpu.max` 값을 읽으면 두 숫자가 나온다.
...
OOM 이벤트가 발생했을 때 커널 로그와 cgroup 이벤트를 확인하는 명령은 다음과 같다.
...
`dmesg` 에 "killed process" 가 보이면 OOM Kill 이 실제로 발생했음을 커널이 기록한 것이다.
...
`cpu.stat` 을 읽으면 throttling 발생 횟수와 총 throttling 시간이 카운터로 기록되어 있다.
...
cgroup 디렉토리를 만들면 커널이 관련 파일을 자동으로 생성한다.
...
디렉토리를 만들자마자 `cpu.max`, `memory.max` 등의 파일이 나타난다.
...
# 다른 터미널에서 top 으로 확인
# stress-ng 가 약 20% CPU 만 사용하는 것이 보인다
```

`top` 에서 stress-ng 가 20% 근방에 머무는 것이 cgroup 절의 "quota 를 다 쓰면 다음 period 까지 대기" 동작이 실제로 집행되는 장면이다.
...
exit code 137 과 `memory.events` 의 `oom_kill 1` 이 Why 에서 다룬 경로 — `memory.max` 초과 → OOM Killer → SIGKILL — 를 수작업으로 재현한 것이다.
...
여기까지 시야도 잘랐고 자원도 잘랐다.

그런데 이것만으로는 격리된 프로세스가 실행 가능한 환경이 되지 않는다.
...
호스트에서 실행 중인 컨테이너의 overlay 마운트 옵션을 직접 확인할 수 있다.
...
`lowerdir` 에 콜론으로 구분된 여러 경로가 있는 것이 각 이미지 레이어가 쌓인 모습이다.
...
두 lowerdir 에 같은 이름의 파일을 두면 우선순위 규칙을 확인할 수 있다.
...
lower1 의 file-b.txt 가 lower2 의 것을 가린 것이 "lowerdir 들을 위에서부터 차례로 검색" 규칙의 실체다.
...
lower1 의 원본이 그대로인 것이 Copy-on-Write 의 증거이며, 수정 시 lowerdir 의 파일이 upperdir 로 복사된 뒤 수정된 것이다.
...
물리적 이더넷 케이블의 양쪽 끝과 동일한 동작이다.

`ip link add` 한 번으로 두 개의 인터페이스가 동시에 생성되며, 한쪽을 컨테이너 namespace 로 이동시킨다.
...
이 한 쌍으로 호스트 namespace 와 컨테이너 namespace 사이의 연결이 만들어지며, CNI 플러그인이 Pod 를 만들 때 가장 먼저 하는 일이 바로 이것이다.
...
`cni0` bridge 를 만들고 각 Pod 의 veth 호스트 쪽 끝을 연결하는 방식이 Flannel 등 브리지 기반 CNI 의 기본 구조다.
...
`conntrack -L` 은 현재 커널이 추적 중인 연결 목록을 보여준다.
...
`/etc/sysctl.d/k8s.conf` 에 기록하는 파라미터와 모듈 로드 명령이다.
...
두 namespace 를 만들고 veth pair 를 연결한다.
...
응답이 오는 것이 veth pair 절에서 다룬 "한쪽 끝으로 들어간 패킷이 반드시 다른 쪽 끝으로 나온다" 의 물리적 실체이며, CNI 플러그인이 Pod 마다 이와 동일한 작업을 수행한다.

bridge 를 추가하여 세 번째 namespace 와의 통신을 확인한다.
...
여기까지 네 원시 자원이 모두 나왔다.
namespace 로 시야를 분리하고, cgroup 으로 자원을 제한하고, OverlayFS 로 파일시스템을 구성하고, veth/bridge/netfilter 로 네트워크를 연결했다.
...
`runc run` 은 같은 디렉토리에 `config.json` 과 `rootfs/` 가 있는 상태에서 OCI Runtime Spec 대로 컨테이너를 실행한다.
...
`config.json` 이 namespace, cgroup, mount 를 모두 선언하며, runc 가 이를 시스템 콜로 번역한다.
...
kubelet 이 CRI 로 명령하고, containerd 가 OCI 로 번역하며, runc 가 syscall 로 실행하는 전체 흐름이다.
...
여기까지 단일 노드 위에서 커널의 네 원시 자원이 OCI/CRI 표준을 통해 컨테이너라는 추상으로 조합되는 전체 경로를 다루었다.
...
VM 생성 후 진입하면 Ubuntu 22.04 환경이 준비된다.

VM 안에서 실습에 필요한 패키지와 커널 모듈을 설치한다.
...
`debootstrap` 으로 최소 Ubuntu rootfs 를 만든다.
...
완료되면 `/tmp/container/rootfs` 에 Ubuntu 최소 파일시스템이 준비된다.

### Step 2 — OverlayFS 마운트

rootfs 를 lowerdir 로, 별도 디렉토리를 upperdir 로 OverlayFS 를 마운트한다.
...
마운트 후 `/tmp/container/merged` 가 컨테이너 rootfs 로 사용할 최종 파일시스템이다.

### Step 3 — cgroup 생성 및 자원 제한

컨테이너에 부여할 CPU 와 메모리 제한을 cgroup 파일에 직접 기록한다.
...
`cpu.max` 와 `memory.max` 파일에 값이 기록되어 자원 제한이 준비된다.

### Step 4 — namespace 분리 + pivot_root + cgroup 적용

`unshare` 로 namespace 를 분리하고 `pivot_root` 로 rootfs 를 전환한 뒤 격리된 셸을 실행한다.
...
`hostname`, `ps`, `/proc/self/cgroup` 이 모두 격리된 값을 보이는 것이 이 편의 핵심 명제 — 컨테이너는 VM 이 아니라 커널 기능의 조합 — 를 직접 증명한 것이다.
...
`ctr` 은 containerd 의 native API 를 직접 호출하며, CRI 를 거치지 않는다.
...
`ctr` 은 containerd 의 native API 를 직접 호출하므로, kubelet 이 사용하는 CRI 경로와는 다른 경로다.

### crictl 로 CRI 시점 확인

`crictl` 은 kubelet 과 동일한 gRPC 경로로 containerd 에 질의한다.
...
`ctr` 은 containerd native 도구이고, `crictl` 은 CRI gRPC 인터페이스를 통해 질의하는 도구로, kubelet 은 `crictl` 과 동일한 gRPC 경로를 사용한다.
...
`Ready` 상태가 나오면 이후 실습을 진행할 수 있다.

### Pod 배포

CPU/메모리 limit 을 지정하여 Pod 를 배포한다.
...
Pod 가 `Ready` 가 되면 네 원시 자원을 역추적할 준비가 된 것이다.

### 원시 자원 1 — Namespace 역추적

`crictl inspect` 로 컨테이너 PID 를 얻고, `/proc/<pid>/ns/` 의 inode 를 호스트와 비교한다.
...
pause 와 app 의 net namespace inode 가 같은 숫자인 것이 namespace 절의 "Pod = pause anchor + setns 합류" 를 실제 값으로 확인한 것이다.

### 원시 자원 2 — cgroup 역추적

`/proc/<pid>/cgroup` 에서 cgroup 경로를 읽고, `cpu.max` 와 `memory.max` 가 Pod spec 대로 설정되었는지 확인한다.
...
`cpu.max` 가 `20000 100000` 인 것이 cgroup 절의 "K8s 의 `cpu: 200m` 이 `20000/100000` 으로 변환된다" 의 물리적 실체다.

### 원시 자원 3 — OverlayFS 역추적

`/proc/<pid>/mountinfo` 에서 overlay 마운트 정보를 읽고, upperdir 에 파일을 써서 CoW 를 확인한다.
...
`overlay-test.txt` 가 upperdir 에서 보이는 것이 OverlayFS 절의 CoW 동작이 실제 Pod 에서도 동일하게 적용됨을 증명한다.

### 원시 자원 4 — 네트워크 역추적

`nsenter` 로 컨테이너 net namespace 안에서 인터페이스를 확인하고, 호스트에서 veth pair 의 다른 쪽 끝을 찾는다.
...
호스트에서 `vethXXXX` 가 보이는 것이 veth 절의 "쌍으로 만들어지는 가상 인터페이스" 가 실제 Pod 에 적용된 모습이다.
...
컨테이너 장애를 추측이 아닌 추적으로 풀 수 있는 것은 이 사다리의 각 층이 `/proc`, `/sys`, `crictl`, `dmesg` 로 관찰 가능하기 때문이다.
