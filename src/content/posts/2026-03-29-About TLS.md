...
Diffie-Hellman 합의에 타원곡선과 임시 키를 결합한 변형이다. 양측이 매 세션마다 임시 키 쌍을 만들어서 공개값(key_share) 만 서로 교환하고, 그 값들을 가지고 수학적으로 같은 값을 각자 계산해 낸다. TLS 1.3 에서는 사실상 표준이며, 2026 시점 ECDHE 의 기본 곡선은 X25519[^rfc7748] 다.

> [!NOTE]
> **(EC)DHE 의 ephemeral 과 forward secrecy**
> 끝의 E (Ephemeral) 는 *매 세션마다 새로 생성되어 세션이 끝나면 폐기되는 일회용 키* 를 의미한다. 서버의 장기 키 (인증서에 묶인 개인키) 가 *키 합의* 에는 사용되지 않으므로, 장기 키가 사후에 유출되어도 과거 세션 트래픽을 복호화할 수 없다 — 이것이 forward secrecy 다. TLS 1.3 에서 RSA 키 전송 모드가 폐기되고 (EC)DHE 만 허용된 이유다. X25519 는 P-256 (secp256r1) 대비 timing attack 이나 invalid curve attack 에 대한 방어가 단순하고, RFC 7748 의 사양이 명확해서 구현 편차가 작다. P-256 과 P-384 는 호환성 fallback 으로 함께 활성화해 둔다. nginx 에서는 다음 한 줄로 우선순위를 지정할 수 있다.
