...
편 3 에서 클러스터의 control plane — etcd 의 Raft 합의, apiserver 의 admission chain, 인증서 3 CA, HA 토폴로지 — 을 다뤘다.

이제 클러스터가 존재하고, scheduler 가 Pod 를 올바른 노드에 배치할 수 있다.

그런데 Pod 가 뜨는 것만으로는 서비스가 동작하지 않는다.

Pod 간에 패킷이 오가야 하고, 클라이언트가 Service IP 로 접근했을 때 올바른 Pod 로 도달해야 한다.

K8s 네트워크에서 가장 흔하게 마주하는 혼란은 다음과 같다.

- `kubectl get svc` 에 나오는 ClusterIP `10.96.50.50` 으로 `curl` 하면 응답이 온다. 그런데 `ip addr show` 에서 이 IP 가 _어디에도 없다_. 어떤 인터페이스에도 할당되어 있지 않은 IP 가 어떻게 동작하는 것인가?
- 같은 노드의 Pod 끼리는 통신되는데, 다른 노드의 Pod 와는 통신이 안 된다. `tcpdump` 를 찍어 보면 패킷이 나가긴 나가는데 돌아오지 않는다. 어디서 막히는 것인가?
- NetworkPolicy 를 적용했는데 _아무 효과가 없다_. YAML 은 맞는데 트래픽이 여전히 통과한다. 무엇이 잘못된 것인가?

이 문제는 _세 개의 다른 네트워크가 동시에 존재한다_ 는 사실에서 비롯된다.

Node 의 네트워크가 있고, Pod 의 네트워크가 있고, Service 의 네트워크가 있다.

각 네트워크는 서로 다른 CIDR 을 사용하고, 서로 다른 컴포넌트가 관리한다.

한 패킷이 이 네트워크들을 거치면서 _src/dst IP 가 도중에 바뀌는데_, 이 변환의 메커니즘을 모르면 디버깅 자체가 불가능하다.

이 편을 읽으면 다음이 가능해진다.

- Service IP 가 인터페이스에 없는데 동작하는 이유를 _iptables NAT 규칙_ 으로 설명할 수 있다.
- Pod 간 통신 장애 시 `tcpdump` → `conntrack` → `iptables-save` 사다리의 어느 단계에서 패킷이 막히는지 추적할 수 있다.
- NetworkPolicy 가 동작하지 않을 때, 그것이 _CNI 플러그인이 enforce 하지 않기 때문_ (Flannel 단독은 미지원) 인지 진단할 수 있다.

### 이 편의 구조

이 편에서는 세 네트워크의 분리에서 시작해, CNI 가 Pod 에 IP 를 부여하는 메커니즘, kube-proxy 가 Service 가상 IP 를 구현하는 NAT 규칙, 패킷이 실제로 거치는 전체 경로, NetworkPolicy 의 enforce 위치, CoreDNS 의 동작까지 — 네트워크 스택의 모든 층을 아래에서 위로 따라간다.
...
왜 이런 원칙이 필요한가. Docker의 기본 네트워크 모델은 호스트 안의 프라이빗 bridge였다.
...
그렇다면 이 약속을 누가 구현하는가. K8s 자체는 구현 방법을 정의하지 않고, **CNI(Container Network Interface)** 표준에 위임한다[^D3].
...
그런데도 이 IP로 통신이 된다. 그 이유는 kube-proxy가 모든 노드의 netfilter에 NAT 규칙을 심어서, 이 IP로 향하는 패킷을 적절한 Pod IP로 변환하기 때문이다.
...
왜일까? 예를 들어 회사 VPC가 `10.244.0.0/16`을 쓰는데 K8s가 같은 대역을 Pod에 할당하면, 패킷이 회사 서버로 가야 할지 클러스터 Pod으로 가야 할지 라우팅 결정이 실패하기 때문이다[^D7].
...
여기까지 Pod에 IP가 부여된다는 사실은 확인했다.
그런데 Pod network의 IP를 누가, 어떻게 Pod에 할당하는가. 이 질문에 답하려면 CNI의 동작을 이해해야 한다.
...
```json
{
    ...
    "ips": [{ "interface": 0, "address": "10.244.1.5/24", "gateway": "10.244.1.1" }],
...
# lo, eth0, flannel.1, cni0, vethXXXX 확인
...
```bash
ip route
# 10.244.X.0/24 via 192.168.x.x dev flannel.1
```
...
여기까지 Pod에 IP가 부여되고 노드 간 라우팅도 구성됐다.
그런데 여전히 의문이 남는다. Service IP는 어떤 인터페이스에도 없는데, 이 가상 IP가 어떻게 동작하는가. 이를 이해하려면 kube-proxy의 메커니즘을 파악해야 한다.
...
왜일까? **kube-proxy**가 모든 노드의 netfilter에 NAT 규칙을 심어 놓았기 때문이다[^D6].
...
kube-proxy는 control plane(규칙을 심는 일)만 담당한다. 따라서 kube-proxy가 죽어도 기존 NAT 규칙은 그대로 동작한다.
...
이 메커니즘이 없으면 어떻게 되는가. 클라이언트는 요청한 적 없는 Pod IP에서 응답을 받게 되어 TCP 연결이 깨진다.
...
여기까지 NAT 변환이 된다는 것은 확인했다.
그런데 변환된 패킷이 실제로 어떤 경로를 거쳐 목적지 Pod에 도달하는가. 같은 노드와 다른 노드에서 경로가 어떻게 달라지는지 알려면 패킷의 전체 여정을 추적해야 한다.
...
```
1. Pod A에서 10.96.50.50:80으로 연결 시도
...
7. 10.244.1.6은 같은 노드 -> cni0 bridge -> veth -> Pod B
...
DNAT가 Pod C를 선택한 경우, 5단계까지는 동일하다.
차이는 그 이후에 발생한다. routing decision에서 `10.244.2.5`가 Node 2에 있으므로 CNI의 라우팅을 탄다.

```
6. nf_conntrack에 NAT 매핑 기록
7. routing decision: 10.244.2.5는 Node 2
8. CNI 라우팅에 따라 패킷 전달
...
출력에서 DNAT 전후의 목적지 IP 변화를 확인할 수 있다.
요청 패킷의 dst가 Service IP에서 Pod IP로 바뀌고, 응답 패킷의 src가 Pod IP에서 Service IP로 바뀐다.
...
conntrack 테이블의 각 행이 양방향 매핑을 동시에 표현한다.
kube-proxy 절의 DNAT와 conntrack 자동 SNAT가 확인된다.

```bash
kubectl delete pod client
```

여기까지 트래픽이 흐르는 경로를 확인했다.
그런데 만약 특정 트래픽만 허용하고 나머지를 차단하려면 어떻게 하는가. 트래픽 경로를 아는 것만으로는 부족하고, 접근 제어 메커니즘이 필요하다.
...
왜 kube-proxy가 아닌가. kube-proxy는 Service 트래픽만 본다.
...
Flannel 클러스터에 NetworkPolicy를 적용하면 어떻게 되는가. apiserver는 NetworkPolicy 객체를 받아주지만, enforce하는 컴포넌트가 없으므로 정책은 무시된다.
...
여기까지 네트워크 접근을 제어하는 방법을 다뤘다.
그런데 아직 한 가지 의문이 남아 있다. Pod가 Service를 IP가 아닌 이름으로 찾는 방법은 무엇인가. 이를 이해하려면 클러스터 DNS의 동작을 파악해야 한다.
...
여기까지는 문제가 없다. 그러나 만약 외부 도메인을 조회하면 어떻게 되는가.
...
개념 절에서 "Flannel 단독은 미지원"이라고 했는데, 그렇다면 Calico에서는 정말로 차단이 동작하는가. 이 실습에서는 default deny 정책과 명시적 허용의 동작을 확인한다.
...
# CoreDNS Pod 로그에서 불필요 쿼리 확인
kubectl logs -n kube-system \
  -l k8s-app=kube-dns --tail=20 \
  | grep google
# google.com.default.svc.cluster.local 같은 쿼리 확인
```
...
```bash
kubectl exec dns-test -- nslookup google.com.
# trailing dot을 붙이면 search 도메인 미적용
# CoreDNS 로그에 불필요 쿼리 없음
```

**Step 5 -- CoreDNS까지의 경로 확인**

```bash
# kube-dns Service의 ClusterIP
kubectl get svc -n kube-system kube-dns

# 그 ClusterIP가 실제 CoreDNS Pod로 DNAT되는 규칙
