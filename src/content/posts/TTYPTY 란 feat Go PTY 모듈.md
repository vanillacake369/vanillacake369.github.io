...
RAW 모드 처리 :: Line Discipline 기능을 비활성화하여 커서 출력이나 화살표 및 키 제어 3.

자식 프로세스 생성 및 FD 설정 4.

쉘 생성 및 PTY Slave 연결 5.
...
부모 프로세스 (Go 프로그램, `xterm` 등)가 실행 2. `pty.Open()`과 같은 시스템 호출을 통해 PTY 쌍을 생성 3.

부모 프로세스는 다음 두 개의 파일 디스크립터(FD) 를 갖게 됨 - **PTY 마스터 FD** (`ptyMaster`) - **PTY 슬레이브 FD** (`ptySlave`)
...
RAW Mode 를 Line Discipline 기능을 비활성화하여 커서 출력이나 화살표 및 키 제어를 사용자가 직접 처리할 수 있도록 한다 2.

이를 위해 두 가지 방법을 처리할 수 있다. 1.

사용자가 직접 키보드로 명령어를 전달 → 부모 프로세스 stdin 에 RAW 설정 2.

프로그 상 Slave 가 PTY 를 통해 명령어 전달 → PTY Slave 의 stdin 에 RAW 설정 3.
...
Consequently, to avoid errors, the child process may only execute async-signal-safe operations until such time as one of the exec functions is called. > > > 이에 따라 go code 안에서 처리되는 thread 간 통신 및 다양한 기능이 처리가 불가능하다. > > 이를 해결하기 위해 아래와 같이 [우회하는 기법](https://stackoverflow.com/a/79722305)이 존재한다. ~~나 또한 이것 이외 더 좋은 방법이 안 떠오른다~~ >
`go
...
PTY Master 는 사용자의 입출력을 받고 2.

커널이 PTY Master 와 PTY Slave 간 양방향으로 데이터로 전달하고 3.
