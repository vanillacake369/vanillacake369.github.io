...
사용자 입력은 기계 키보드에서 UART(Universal Asynchronous Receiver and Transmitter)를 통해 

컴퓨터로 전송되었고, 출력은 다시 UART를 통해 사용자에게 인쇄되는 방식이었다. 
...
사용자가 로그인하면 해당 TTY 장치 파일의 소유권이 해당 사용자에게 넘어가고, 

이후 프로세스는 이를 통해 TTY 동작을 제어할 수 있다.
...
즉 요컨데 우리가 흔히 쓰는 터미널 에뮬레이터는 userland process , 사용자의 process 이다.
...
master 는 터미널 에뮬레이터 프로세스와 입출력을 처리하고, tty driver 와 line descipline 이 slave 로 해당 내용을 중계해준다.
...
프로세스 생성과정이 필요하다.  여기서 부모프로세스와 자식프로세스를 통해 PTY Master 와 PTY Slave 를 처리한다.

여기서 부모는 PTY Master 를 가지고 터미널 에뮬레이터에 대한 처리를 하며, 자식은 PTY Slave 를 통해 표준 입출력을 처리한다.

1. 부모 프로세스 시작
2. RAW 모드 처리 :: Line Discipline 기능을 비활성화하여 커서 출력이나 화살표 및 키 제어
3. 자식 프로세스 생성 및 FD 설정
4. 쉘 생성 및 PTY Slave 연결
5. 시그널에 따른 PTY 처리
...
1. 부모 프로세스 (Go 프로그램, `xterm` 등)가 실행
2. `pty.Open()`과 같은 시스템 호출을 통해 PTY 쌍을 생성
3. 부모 프로세스는 다음 두 개의 파일 디스크립터(FD) 를 갖게 됨
...
1. RAW Mode 를 Line Discipline 기능을 비활성화하여 커서 출력이나 화살표 및 키 제어를 사용자가 직접 처리할 수 있도록 한다
2. 이를 위해 두 가지 방법을 처리할 수 있다.
    1. 사용자가 직접 키보드로 명령어를 전달 → 부모 프로세스 stdin 에 RAW 설정
    2. 프로그 상 Slave 가 PTY 를 통해 명령어 전달 → PTY Slave 의 stdin 에 RAW 설정 
3. PTY Slave TTY 원래 설정을 저장했다가 PTY Slave 가 종료 시 원래 설정으로 복구시켜 정리한다.
...
- 자식 프로세스는 exec() 를 통해 자기 자신을 쉘 프로세스로 교체한다. 이를 통해 PTY Slave 를 쉘에 연결한다.
...
위에서 살펴본 각 단계들에 대한 go 에서 처리할 함수들은 다음과 같다.
...
3. 자식 프로세스 생성 및 FD 설정
    ...
    자식 프로세스를 생성하고 — go 에서는 fork 를 사용할 수 없다. 이유는 아래를 참고 — 
...
4. 쉘 생성 및 PTY Slave 연결
    ...
    1. PTY Master 는 사용자의 입출력을 받고
    2. 커널이 PTY Master 와 PTY Slave 간 양방향으로 데이터로 전달하고
    3. PTY Slave 는 자식 쉘 프로세스에서 이를 처리하는 구조이다.
