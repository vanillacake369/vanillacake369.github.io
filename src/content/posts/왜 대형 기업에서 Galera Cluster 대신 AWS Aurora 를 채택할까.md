...
- <span style="color:yellowgreen">**Multi-threaded Slave**</span> For better performance.
...
Slave Node에서는 이 BIN LOG를 복사해온다.

이 복사 작업을 IO Thread라는 스레드가 수행하는데, 이렇게 읽어온 내용은 Replay Log라는 파일에 기록이 된다.
...
신뢰도가 높은 반면, 단점으로는 읽기와 쓰기 노드를 분리해야 한다. 2.

데이타 복제가 동기 방식이 아닌 비동기 방식으로 적용되기에, master node에 적용한 데이타 변경사항이 slave에 반영될때까지 일정 시간이 걸린다.

이로 인해 master와 slave node간의 순간적인 데이타 불일치성이 발생할 수 있다.
...
여기서 Synchronous Replication 을 적용하리란 쉽지 않다.

성능 저하로 이어질 수 있기 때문이다.
