...
인프라를 코드로 관리하는 IaC 시대지만, 홈랩을 운영하다 보면 여전히 해결되지 않는 운영의 사각지대 를 마주하게 된다.

Terraform 으로 가상 머신을 프로비저닝하더라도, 그 안의 커널 파라미터를 수정하거나 특정 시스템 서비스를 구성하는 일은 여전히 명령형 스크립트나 관리자의 기억에 의존하곤 한다.

Terraform 과 OpenTofu 는 클라우드 리소스의 선언적 관리에는 탁월하지만, OS 안쪽 — 어떤 커널 모듈을 올릴 것인지, `/etc` 의 어떤 파일이 누구에 의해 관리되는지, 시스템 업그레이드가 atomic 한지 — 까지는 책임지지 못한다.

이 영역은 여전히 Ansible/Chef 같은 명령형 도구로, 혹은 수동 bash 스크립트로 메워지고 있다.

NixOS 는 정확히 이 사각지대를 메우는 도구다.

시스템의 모든 상태를 단 하나의 `.nix` 파일로 정의하고, 이를 함수의 출력값 처럼 선언적으로 관리할 수 있기 때문이다.

그러나 단순히 "좋은 도구라서" 라는 이유만으로 NixOS 기반의 K8s 클러스터 구축을 시작한 것은 아니다.

### 선언적 설계의 한계 확인

K8s 는 OS 레벨의 설정에 매우 민감한 워크로드다.

kubelet 은 커널 모듈과 sysctl 파라미터를 직접 요구하고, containerd 는 cgroup 드라이버가 정확히 맞아야 동작하며, Cilium 같은 CNI 는 BPF 파일시스템이라는 특수한 환경을 필요로 한다.

이처럼 까다로운 요구사항들을 NixOS 특유의 읽기 전용 `/nix/store` 와 중앙 집중식 설정 체계 안으로 편입시키는 과정은, NixOS 의 선언적 설계가 복잡한 분산 시스템을 어디까지 수용할 수 있는지 확인하는 작업이었다.

### 추상화 이면의 원리 파악

물론 EKS 나 GKE 같은 매니지드 서비스를 사용하면 이러한 OS 레벨의 복잡성은 가려진다.

그러나 가려졌다 는 것이 필요 없다 는 뜻은 아니다.

kubelet 이 projected volume 마운트에 실패하거나 CNI 가 Pod 에 IP 를 할당하지 못하는 실무의 장애 상황에서, 문제 해결의 열쇠는 결국 OS 와 K8s 가 만나는 접점의 원리를 얼마나 이해하고 있느냐에 달려 있다.

홈랩에서 밑바닥부터 클러스터를 구축하는 과정은, 편리한 추상화에 가려져 있던 K8s 의 내부 동작 원리를 직접 확인할 수 있는 방법이다.

### 이 글의 성격 — 이상과 현실의 간극을 메운 기록

이 글은 단순히 성공적인 설치 단계를 나열한 가이드가 아니다.

모든 것을 선언적으로 관리하려는 NixOS 의 설계와, 동적으로 상태를 변경하려는 K8s 구성 요소들 사이에서 발생한 실제 장애 — 특히 kubelet 의 Mount Namespace 이슈 — 를 어떻게 논리적으로 추론하고 해결했는지에 대한 기록이다.

각 원리 영역에 대한 상세 딥다이브는 별도의 글로 분리해 두었으며, 이 글에서는 tonys-homelab 의 실제 구축 맥락 과 NixOS 특화 이슈 에 집중한다.

---

# What? 뭘 배움?

## 들어가며 — 물리 서버 한 대에서 K8s 클러스터까지의 아키텍처 🪜
...
이 아키텍처가 실제로 어떤 흐름으로 배포되는지를 이해하려면, 각 도구가 어떤 계층을 담당하는지를 함께 봐야 한다. 다음 흐름도는 개발 머신에서 GitOps 동기화까지의 전체 배포 파이프라인을 보여준다.
...
## NixOS 에서 K8s 를 돌리기 위한 호환성 계층 — FHS 심링크와 crictl 설정 🐧
...
왜냐하면 kubeadm 의 preflight check 와 kubelet 런타임은 이 FHS 경로들을 _하드코딩_ 으로 기대하기 때문이다[^K1].

NixOS 에서 `kubeadm init` 을 실행하면 `socat not found` 에러가 나는 이유가 바로 이것이다.

`atoms/k8s/k8s-distro-compat.nix` 가 이 간극을 메운다[^K2].

```nix
# atoms/k8s/k8s-distro-compat.nix (핵심부)
...
FHS 호환 계층이 갖춰졌다면, 다음 단계는 K8s 가 실제로 컨테이너를 실행할 수 있는 커널 환경을 준비하는 것이다.
...
이 원리에 대한 상세 설명은 별도의 딥다이브 글을 참고한다[^K3].

`atoms/k8s/container-runtime.nix` 에서 이 의존성을 선언한다[^K4].
...
| 설정                      | 역할                                                                      |
| ------------------------- | ------------------------------------------------------------------------- |
| `overlay`                 | OverlayFS 커널 모듈. containerd 가 이미지 레이어를 합성할 때 사용         |
| `br_netfilter`            | bridge 트래픽이 iptables/nftables 를 거치게 함. CNI 의 Pod 간 통신에 필요 |
| `net.ipv4.ip_forward`     | 노드가 라우터로 동작. Pod 패킷을 다른 노드로 전달                         |
| `bridge-nf-call-iptables` | bridge 위의 패킷에 netfilter 규칙 적용                                    |
...
즉 kubelet 은 `systemd` 방식으로 cgroup 경로를 탐색하는데, containerd 가 `cgroupfs` 방식으로 다른 경로에 cgroup 을 만들면 서로 엇갈리는 것이다.
...
이 설정이 왜 필요한지는 다음 절이 아닌, Mount Namespace 장애 절에서 다룬다.
...
이 전에는 kubelet 이 시작되어도 설정이 없으므로 실패한다.
...
이 경우 kubelet 이 ConfigMap/Secret 에 대해 LIST/WATCH 요청을 보내는데, Node authorizer 가 이를 거부한다[^K7].

왜냐하면 Node authorizer 는 보안상 kubelet 이 자신의 노드에 할당된 리소스만 접근하도록 제한하는데, LIST/WATCH 는 클러스터 전체 리소스를 대상으로 하기 때문이다.
...
## Mount Namespace 장애 — projected volume 마운트가 컨테이너에 전달되지 않는 원인과 해결 🔍

지금까지 NixOS 의 FHS 호환, 컨테이너 런타임, kubelet 서비스를 다뤘다.

이 설정들만으로 클러스터가 정상 동작해야 하지만, tonys-homelab 에서는 그렇지 않았다.

클러스터 부트스트랩 직후 모든 Pod 가 시작에 실패하는 장애가 발생했다.

이 절에서는 그 장애의 원인과 해결을 다루며, NixOS 에서 K8s 를 운영할 때 가장 주의해야 할 지점을 드러낸다.
...
ServiceAccount 토큰 발급과 projected volume 의 상세 원리는 별도 글을 참고한다[^K5].
...
이 옵션들은 서비스 프로세스에 **별도의 mount namespace** 를 생성한다[^K8]. 그 결과 kubelet 과 containerd 가 각각 격리된 파일시스템 뷰를 갖게 된다.
...
결과적으로 bind mount 는 성공하지만, 컨테이너 안에 빈 디렉토리가 마운트된다. 앞서 정상 흐름도에서 "같은 namespace → 마운트 전파" 라고 설명한 전제가 깨지는 것이다.
...
upstream kubeadm 환경에서도 kubelet 은 host mount namespace 에서 동작한다[^K9].
...
kubelet 과 containerd 가 같은 mount namespace 에서 동작하도록 설정했다면, 다음으로 해결해야 할 문제는 네트워크다. K8s 클러스터에는 세 개의 분리된 네트워크가 존재하며, 이 세 네트워크의 CIDR 을 일관되게 관리하지 않으면 라우팅이 깨진다.
...
세 네트워크의 원리와 패킷 흐름에 대한 상세 설명은 별도 글을 참고한다[^K10].

tonys-homelab 은 `network/topology.nix` 에서 모든 네트워크 상수를 중앙 관리한다[^K11].

```nix
# network/topology.nix (핵심부)
{
  vlans.services = {
    id = 20;
...
Ansible 이 `topology.nix` 의 데이터를 사용하는 방식이 `inventory.nix` 다[^K12].

```nix
# ansible/inventory.nix (핵심부)
roleOf = name:
...
`inventory.py` 가 이 Nix 파일을 `nix eval --json` 으로 평가해 Ansible 에 넘긴다. 이 구조 덕분에 Nix 와 Ansible 사이에 값이 하드코딩으로 중복되는 일 없이, topology.nix 의 변경이 양쪽 모두에 자동 전파된다.
...
CNI 가 Pod 에 IP 를 할당하는 메커니즘과 kube-proxy 의 NAT 원리에 대한 상세 설명은 별도 글을 참고한다[^K10].
...
`atoms/k8s/cni-cilium.nix` 에서 BPF 파일시스템과 cgroup v2 를 마운트한다[^K13].
...
Ansible 의 `roles/cni/tasks/main.yml` 이 Cilium 을 Helm 으로 설치한다[^K14].

```yaml
# roles/cni/tasks/main.yml (핵심 values)
values:
    ipam:
        mode: "cluster-pool"
        operator:
...
| 설정                         | 의미                                                         |
| ---------------------------- | ------------------------------------------------------------ |
| `kubeProxyReplacement: true` | kube-proxy DaemonSet 불필요. Service NAT 를 eBPF 로 처리     |
| `routingMode: native`        | VXLAN 오버레이 없이 직접 라우팅. 같은 L2 네트워크이므로 가능 |
| `autoDirectNodeRoutes: true` | 각 노드의 Pod CIDR 에 대한 라우트를 자동 설치                |
| `bpf.masquerade: true`       | Pod → 외부 트래픽의 SNAT 을 eBPF 로 처리                     |
...
NixOS 설정과 Cilium 사전 준비까지 완료했다면, 이제 실제로 클러스터를 생성하는 단계다. control plane 의 네 컴포넌트 (apiserver, etcd, scheduler, controller-manager) 와 Raft 합의, 인증서 체인에 대한 상세 설명은 별도 글을 참고한다[^K15].

Ansible 의 `roles/kube-node/tasks/main.yml` 이 7단계로 부트스트랩을 수행한다[^K16].
...
이 두 상태에 따라 이후 phase 의 실행 여부가 결정된다. 예를 들어 이미 join 된 노드에서는 Phase 3 (Leader Init) 과 Phase 5 (Node Join) 를 건너뛰고, 클러스터가 아직 존재하지 않으면 Phase 3 에서 최초 초기화를 수행한다.
...
kube-vip 는 static Pod 로 떠야 하는데, static Pod 가 뜨려면 kubelet 이 돌아야 하고, kubelet 이 돌려면 kubeadm init 이 끝나야 한다. 이것이 전형적인 닭과 달걀 문제다.

해법은 _임시 VIP 바인딩_ 이다[^K17].
...
1. 수동으로 VIP 를 인터페이스에 바인딩
2. kubeadm init 실행 (VIP 가 로컬이므로 kubeconfig 가 동작)
3. kube-vip static Pod manifest 를 `/etc/kubernetes/manifests/` 에 배치
4. kubelet 이 kube-vip Pod 를 시작하고 VIP 를 인수
5. kube-vip 가 VIP 를 관리하는 것을 확인한 뒤 수동 바인딩 제거

### kubeadm 설정

`kubeadm-config.yaml.j2` 의 핵심 부분이다[^K18].
...
NixOS 의 systemd-resolved 가 `/etc/resolv.conf` 를 `127.0.0.53` 스텁으로 설정하는데, 이 값이 Pod 내부에 주입되면 DNS 가 깨지기 때문이다. `127.0.0.53` 은 호스트의 systemd-resolved 를 가리키지만, Pod 네트워크 namespace 안에서는 이 주소에 아무 서비스도 없으므로 DNS 질의가 모두 실패한다.

---

## 역할 조합 — atoms 를 role 로 묶는 NixOS 모듈 구조 📦

지금까지 다룬 4개의 atom — FHS 호환, 컨테이너 런타임, kubelet 서비스, Cilium CNI — 은 각각 독립된 `.nix` 파일이다. 이 atom 들을 역할별로 조합하는 것이 `nodes/roles/` 디렉토리의 역할이다.
...
위에서 다룬 4개의 atom 이 `nodes/roles/k8s-master.nix` 에서 조합된다[^K19].

```nix
# nodes/roles/k8s-master.nix
{pkgs, ...}: {
  imports = [
    ./common.nix
    ../../atoms/k8s/container-runtime.nix
...
```nix
# nodes/vms/k8s-master-1.nix (핵심부)
node = {
  ip = net.ip;
  mac = net.mac;
  role = "k8s-master";
...
topology.nix 가 세 CIDR 을 중앙 관리한다고 설명했지만, 실제로 겹침이 없는지는 직접 값을 출력해 봐야 확신할 수 있다. 이 실습에서는 `nix eval` 로 topology.nix 의 네트워크 상수를 추출하고, Ansible 인벤토리가 같은 값을 참조하는지까지 검증한다.
...
`10.0.20.x`, `10.244.x.x`, `10.96.x.x` — 세 대역이 겹치지 않음을 확인한다. 이 세 값이 서로 다른 주소 공간에 있다는 것은, Node 트래픽과 Pod 트래픽과 Service 트래픽이 라우팅 테이블에서 명확히 구분된다는 뜻이다. 만약 Pod CIDR 이 Node CIDR 과 겹친다면, 노드가 Pod 행 패킷을 자기 자신의 로컬 네트워크로 착각하여 CNI 를 거치지 않고 직접 전달하려 할 것이다.
...
topology.nix 의 값과 Ansible 인벤토리의 값이 일치한다. "단일 진실 원천" 설계의 효과다. inventory.nix 가 topology.nix 를 import 하기 때문에, 두 도구 사이에 값이 어긋날 여지가 원천 차단된다.
...
세 값이 다르면 mount namespace 격리가 원인이다. Mount Namespace 장애 절에서 다룬 "kubelet 과 containerd 가 서로 다른 mount namespace 에서 동작하면 마운트가 전파되지 않는다" 는 원리가 여기서 확인된다. kubelet 의 namespace ID 가 host (PID 1) 와 다르다는 것은, kubelet 이 생성한 tmpfs 마운트가 host 에서 보이지 않음을 의미한다.
...
kubelet 의 mount namespace ID 가 host 의 것 (`4026531840`) 과 동일해졌다. 이 상태에서 kubelet 이 생성하는 tmpfs 마운트는 host mount namespace 에서도 보이므로, containerd 가 이를 컨테이너 안으로 정상적으로 bind mount 할 수 있다.

## 실습 ③ — etcd snapshot 백업과 복구 🗄️

etcd 의 Raft 합의와 쿼럼에 대한 원리는 별도 글을 참고한다[^K15].
...
3 CA 체계와 mTLS 에 대한 원리는 별도 글을 참고한다[^K15].
...
[^K1]: kubeadm preflight checks — socat, conntrack 등 FHS 경로 요구. <https://kubernetes.io/docs/reference/setup-tools/kubeadm/implementation-details/#preflight-checks>

[^K2]: tonys-homelab k8s-distro-compat.nix. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/atoms/k8s/k8s-distro-compat.nix>

[^K3]: 편 1 — Linux 원시 자원과 컨테이너 (namespace, cgroup, OverlayFS 딥다이브).

[^K4]: tonys-homelab container-runtime.nix. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/atoms/k8s/container-runtime.nix>

[^K5]: 편 2 — 단일노드와 Pod (kubelet, ServiceAccount, Volume 딥다이브).

[^K6]: tonys-homelab kubelet-service.nix. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/atoms/k8s/kubelet-service.nix>

[^K7]: Kubernetes Node Authorization — kubelet 의 API 접근 제한. <https://kubernetes.io/docs/reference/access-authn-authz/node/>

[^K8]: systemd.exec(5) — PrivateTmp, PrivateMounts, ProtectSystem. <https://www.freedesktop.org/software/systemd/man/latest/systemd.exec.html>

[^K9]: Kubernetes kubelet source — volume manager 는 host mount namespace 전제. <https://github.com/kubernetes/kubernetes/blob/master/pkg/kubelet/volumemanager/volume_manager.go>

[^K10]: 편 4 — 클러스터 네트워크 (CNI, kube-proxy, Service, NetworkPolicy 딥다이브).

[^K11]: tonys-homelab topology.nix. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/network/topology.nix>

[^K12]: tonys-homelab inventory.nix. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/ansible/inventory.nix>

[^K13]: tonys-homelab cni-cilium.nix. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/atoms/k8s/cni-cilium.nix>

[^K14]: tonys-homelab Cilium Ansible role. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/ansible/roles/cni/tasks/main.yml>

[^K15]: 편 3 — 클러스터와 분산 상태 (apiserver, etcd/Raft, 인증서, HA 딥다이브).

[^K16]: tonys-homelab kube-node main.yml. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/ansible/roles/kube-node/tasks/main.yml>

[^K17]: tonys-homelab init-leader.yml. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/ansible/roles/kube-node/tasks/init-leader.yml>

[^K18]: tonys-homelab kubeadm-config.yaml.j2. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/ansible/roles/kube-node/templates/kubeadm-config.yaml.j2>

[^K19]: tonys-homelab k8s-master.nix. <https://github.com/vanillacake369/tonys-homelab/blob/572a89c/nodes/roles/k8s-master.nix>
