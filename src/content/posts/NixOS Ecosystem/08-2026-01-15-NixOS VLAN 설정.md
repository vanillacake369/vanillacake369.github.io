...
홈랩 서버 한 대를 NixOS 로 운영하면서 관리망과 서비스망을 분리하고 싶었다. 다만 물리 NIC 가 한 포트뿐이라 추가 스위치 없이 같은 케이블 위에 두 네트워크를 태깅으로 얹어야 했다. NixOS 의 `networking.vlans` 와 `networking.bridges` 옵션이 정확히 무엇을 만들고, OPNsense VM 의 트렁크 인터페이스와 어떻게 결합되어 게이트웨이 노릇을 하는지 한 줄의 그림으로 정리할 필요가 있었다.

이 글은 VLAN 의 기본 개념, 단일 NIC Router-on-a-Stick 구성, NixOS 의 bridge/vlan/interface 선언, OPNsense 게스트의 트렁크 수신 설정, 그리고 실습 환경에서 동일한 구성을 NixOS VM 두 대로 재현하는 검증까지 한 번에 정리한다.
...
게스트 측 설정은 OPNsense 의 웹 UI 에서 다음 순서로 진행한다.
