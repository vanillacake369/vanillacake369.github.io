...
- if an instance is <span style="color: YellowGreen"> already persistent, then this call has no effect for this particular instance ***(but it still cascades to its relations with cascade=PERSIST or cascade=ALL).***  </span> 
...
- if the entity is  <span style="color: YellowGreen"> transient, it copies upon a newly created persistent entity.
  ***this operation cascades for all relations with cascade=MERGE or cascade=ALL mapping.*** </span>
- if the entity is  <span style="color: YellowGreen"> persistent, then this method call doesn’t have an effect on it (***but the cascading still takes place).*** </span>
