---
description: "WebClient API 호출 코드에서 발생하는 중복을 제네릭 인터페이스와 함수형으로 제거한 리팩터링 과정을 정리한다."
date: 2024-10-01
tags: [journal]
lang: ko
draft: false
...
사내에서는 논블로킹 API 호출을 처리하고자 Webflux & Webclient 를 사용 중에 있다. Webflux & Webclient 를 사용하면 세 가지 반환 타입을 활용할 수 있다.

- block 처리를 하여 결과값을 그대로 반환하거나,
- 하나의 스트림을 담는 Mono 를 반환하거나
- 여러 스트림을 담는 Flux 를 반환할 수 있다.

하지만 기획이 지속적으로 변경됨에 따라 어떤 변경점이 있을지 몰랐고, 메서드 오버로딩을 통해 세 가지 모두 처리하게끔 하였다.
...
하지만 API 호출이 많아질수록 중복코드가 늘어났다. 각 API 호출 별로 모두 동일 옵션에 대해 동일한 정책을 취했다.
...
- API 호출 정책에 따라 API 호출을 처리한다.
  ...
  - `retry()`를 통해 error 발생 시 3번의 Re-subscribes 한다.[^2]
  - `subscribeOn()`을 통해 subscriber 에 대한 scheduler 를 등록한다.[^3]
...
이를 통해 조금 더 읽기 수월하게 구현할 수 있었고, 중복코드를 줄일 수 있었다. 중복코드가 줄다보니 구현하기도 수월해져 구현시간이 단축되는 효과도 있었다.
...
단점이라고 한다면 API 호출 별로 정책이 획일화되어있다는 것이다. 만약 예민한 API 호출이라면 API 의 TPS 에 따라 정책을 달리해야 할 것이다.[^4]

[^1]: Java 8 이후 `interface` 에 `default` 메서드를 선언할 수 있어 공통 로직을 인터페이스 레벨에서 제공할 수 있다. 구현체는 필요한 경우 `default` 메서드를 오버라이드하여 정책을 재정의할 수도 있다.
[^2]: `retry(3)` 은 업스트림에서 에러 시그널이 발생했을 때 최대 3회 재구독(re-subscribe)을 시도한다. 멱등성이 보장되지 않는 API 에서는 재시도 횟수를 신중하게 설정해야 한다.
[^3]: `subscribeOn(Schedulers.boundedElastic())` 은 블로킹 I/O 작업을 전용 스레드 풀에서 처리하도록 한다. Reactor 의 이벤트 루프 스레드를 블록하지 않기 위해 WebClient 와 함께 자주 사용되는 패턴이다.
[^4]: API 별 정책 분기가 필요한 경우, `fetchMonoInternal` 의 파라미터로 `Duration timeout`이나 `int retryCount` 를 추가로 받도록 오버로드하는 방식으로 확장할 수 있다.
[^5]: `ParameterizedTypeReference<T>` 는 제네릭 타입 정보가 런타임에 소거(type erasure)되는 문제를 우회하기 위해 Spring 에서 제공하는 유틸리티다. `List<Foo>` 처럼 중첩된 제네릭 타입을 역직렬화할 때 필수적으로 사용된다.
