...
1. **Docker 소켓 접근**: **`volume=/var/run:/var/run:ro`** 마운트는 cAdvisor가 Docker 데몬 소켓에 접근할 수 있게 하여 Docker와 통신하고 호스트에서 실행 중인 모든 컨테이너를 탐색할 수 있게 합니다. running.md:9-17
2. **루트 파일시스템 접근**: **`volume=/:/rootfs:ro`** 마운트는 호스트의 전체 파일 시스템에 대한 접근을 제공하며, cAdvisor는 이를 통해 컨테이너 데이터를 읽습니다.
3. **Cgroup 접근**: **`volume=/sys:/sys:ro`** 마운트는 cAdvisor가 모든 컨테이너의 cgroup 정보를 읽을 수 있도록 합니다.
