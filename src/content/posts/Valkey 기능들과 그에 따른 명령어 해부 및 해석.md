...
OSSCA 를 통해 강대명 Valkey 컨트리뷰터님과 Valkey 오픈소스 컨트리뷰션 세션을 진행하고 있다.
1 주차에는 Git 내부 처리 과정을 익혔고, 2-3 주차를 통해 Valkey/Redis 의 철학과 기본 자료구조/명령어를 알아보았다.
컨트리뷰터님께서 숙제로 Valkey 실행해보고, RESP 와 기본자료구조/명령어를 실습하는 것을 내주셨다.
본 포스트에서는 기본 자료구조와 처리 과정, 실제 명령어를 실습해보고 Valkey 내부를 해부해본다

## Valkey — Redis 라이선스 분기 후의 새 본진 🗂️

Valkey 는 2024 년 3 월, Redis Inc. 가 Redis 의 라이선스를 BSD 에서 SSPL/RSALv2 로 변경하면서 시작된 fork 다[^V1].
...
실서비스에서 Valkey 의 성능과 메모리 효율은 내부 자료구조 선택 에서 직접 결정된다.
다음은 자주 마주치는 사례들이다.
...
내부를 알면 같은 데이터로 Valkey 메모리를 절반으로 줄일 수도 있고, 
...
```
명령어                     ← 사용자가 valkey-cli 에 입력
  └ RESP 프로토콜          ← 서버-클라이언트 간 직렬화 형식
      └ robj               ← 모든 값을 감싸는 공통 wrapper
          └ encoding       ← 자료구조의 물리적 표현 선택
              └ C 자료구조 ← SDS, listpack, hashtable, skiplist 등
```

이 사다리를 위에서 아래로 따라가면서 각 층이 다음 층을 어떻게 호출하는지 본다.
`SET key value` 라는 단순한 명령 한 줄이 어떻게 RESP 로 직렬화되어 서버에 도달하고, robj 로 감싸져 어떤 인코딩이 선택되며, 결국 어떤 C 자료구조에 저장되는가를 추적할 수 있게 된다.
다음 절부터는 이 사다리를 위에서부터 차례로 내려간다.

## RESP — 텍스트 기반 직렬화 프로토콜 📜

RESP (REdis Serialization Protocol) 는 Valkey 클라이언트와 서버가 주고받는 메시지의 형식을 정의한다[^V2].
...
RESP3 (Redis 6 도입) 은 Map, Set, Boolean, Double, Big Number, Null, Verbatim String 등 더 풍부한 타입을 추가했다[^V3].
Valkey 는 두 버전 모두 지원하며, 클라이언트가 `HELLO 3` 명령으로 RESP3 로 업그레이드할 수 있다.
대부분의 운영 환경은 여전히 RESP2 를 사용한다.
다음 절에서는 RESP 로 도착한 명령이 서버 안에서 처리될 때 모든 값이 거치는 공통 wrapper, robj 를 본다.
...
전체 크기는 16 바이트로 cache line 의 1/4 이다[^V4].
...
| `refcount` | 32 비트 | 공유 객체의 참조 카운트 (예: 정수 0–9999 는 공유 풀) |
...
`OBJECT ENCODING <key>` 로 해당 키의 현재 encoding 을 확인할 수 있다[^V5].
...
이 명령은 자료구조 동작을 검증하는 가장 직접적인 도구이며, How 절의 모든 실습에서 활용된다.
다음 절부터는 각 type 별로 어떤 encoding 들이 있고 내부 자료구조가 어떻게 동작하는지 본격적으로 분해한다.


## String — SDS 자료구조와 세 가지 인코딩을 활용한 key-value 저장소 🔍

직전 절에서 모든 값이 `robj` 라는 wrapper 에 들어가며 `type` 과 `encoding` 이 별도 필드로 분리되어 있다는 것을 봤다.
가장 단순한 type 인 String 부터 그 분리가 어떻게 활용되는지 본다.

### 세 가지 인코딩

`type=OBJ_STRING` 은 다음 세 가지 중 하나의 `encoding` 을 가진다[^V6].
...
`OBJECT ENCODING` 명령으로 직접 확인할 수 있다.
...
embstr 의 디자인은 `robj` 와 SDS 헤더, 그리고 실제 데이터를 모두 한 cache line 안에 담는 것을 목표로 한다[^V8].
...
44 라는 값은 `64 - 16 (robj) - 3 (sdshdr8 헤더) - 1 (null 종결자)` 의 결과다[^V9].

### SDS — Valkey 의 자체 문자열 타입

embstr 과 raw 의 데이터 부분은 모두 **SDS (Simple Dynamic String)** 라는 Valkey 자체 문자열 타입을 사용한다[^V10].
C 표준 문자열 (null 종결 문자 배열) 은 단순하지만 다음 한계를 갖는다.

| 한계 | 영향 |
| --- | --- |
| 길이를 알려면 끝까지 순회 | `strlen` 이 O(N) |
| binary safe 하지 않음 | 데이터 안에 `\\0` 이 있으면 그곳에서 잘림 |
| 할당 크기를 모름 | 버퍼 오버런 위험 |
...
`buf` 앞에 `len` 과 `alloc` 이 사전에 저장된다[^V11].
이 구조로 다음 동작이 가능해진다.

| 동작 | 구현 |
| --- | --- |
| `sdslen()` 이 O(1) | `len` 필드를 직접 읽음 |
| binary safe | `buf` 안에 `\\0` 이 있어도 `len` 으로 길이 판단 |
| 오버런 검사 가능 | `alloc - len` 으로 남은 공간 계산 |
| 기존 C 코드와 호환 | `buf` 가 `\\0` 종결되어 `printf("%s", sds)` 가 그대로 동작 |
...
append 시 메모리 재할당 횟수를 줄이는 SDS 의 핵심 함수다[^V13].
...
Valkey 7+ 에서 List 의 인코딩은 `OBJ_ENCODING_QUICKLIST` 단 하나다[^V14].
...
작은 List 든 큰 List 든 모두 quicklist 다.
...
listpack 은 Redis 7.0 에서 ziplist 를 대체한 새로운 컴팩트 배열 인코딩이다[^V15].
...
quicklist 에 임의 위치로 삽입하는 함수다[^V16].
...
핵심 로직은 현재 노드의 listpack 에 공간이 있으면 거기에, 없으면 새 노드를 만들어 분할한다.
대량의 LPUSH/RPUSH 가 일어날 때 quicklist 는 양 끝의 listpack 을 채우다가 한계에 도달하면 새 노드를 추가하는 식으로 늘어난다.
다음 절은 키-값 매핑을 표현하는 Hash 자료구조다.
작은 Hash 와 큰 Hash 가 어떻게 다른 내부 표현으로 전환되는지, 그리고 그 전환 임계가 어떻게 결정되는지 본다.

## Hash — listpack 과 hashtable 인코딩 기반 필드-값 맵 🔍

직전 절에서 List 가 quicklist 라는 단일 인코딩으로 동작하지만 그 안에 listpack 을 결합하는 패턴을 봤다.
...
`type=OBJ_HASH` 는 다음 두 인코딩 중 하나를 가진다[^V17].
...
둘 중 하나라도 초과하면 hashtable 로 전환된다[^V18].
`OBJECT ENCODING` 으로 직접 확인할 수 있다.
...
값 하나가 64 byte 를 넘는 순간 hashtable 로 전환되었다.
...
listpack 에서 hashtable 로의 전환 함수다[^V19].
...
직전 절에서 Hash 가 listpack 과 hashtable 두 인코딩 사이를 자동 전환한다는 것을 봤다.
...
`type=OBJ_SET` 은 다음 세 인코딩 중 하나를 가진다[^V20].
...
intset 은 정렬된 정수 배열이며, 이진 탐색으로 O(log N) 멤버 확인이 가능하다[^V21].
...
특이한 점은 contents 의 크기가 멤버 값에 따라 달라지는 것이다.
...
직전 절까지 본 Hash 와 Set 는 한 시점에 한 인코딩만 사용했다.
...
큰 ZSet 의 `OBJ_ENCODING_SKIPLIST` 는 두 자료구조를 함께 사용하는 특이한 구조다[^V23].
...
같은 데이터를 두 자료구조에 중복 저장하지만, 메모리는 멤버 SDS 자체는 공유한다 (포인터만 두 곳에서 가리킴).
...
ZSet 의 정렬 자료구조로 RB-tree (red-black tree) 가 아닌 skiplist 가 선택된 것은 antirez 의 명시적 결정이다[^V24].
...
다음 절은 위 자료구조들의 공통 기반인 dict 그 자체를 본다.
...
직전 절들에서 Hash, Set, Sorted Set 이 모두 hashtable 인코딩으로 전환될 수 있다는 것을 봤다.
...
Valkey 는 이를 작은 단위로 나누어 점진적으로 처리하는 incremental rehashing 으로 해결한다[^V26].
...
다음 절은 시간 순서로 데이터를 추가하는 Stream 자료구조다.
...
직전 절들에서 본 자료구조들은 모두 임의 위치 갱신을 지원했다.
Stream 은 그와 다르게 append-only 시간 정렬 로그다.

### Stream 의 역할

Stream 은 Redis 5.0 (2018) 에 도입된 자료구조로, Kafka 와 유사한 메시지 큐 / 이벤트 로그 패턴을 표현한다[^V28].
...
내부적으로는 radix tree 와 listpack 의 결합으로 구성된다[^V29].
...
stream 에 새 entry 를 추가하는 함수다[^V30].
...
다음 절은 마지막 자료구조 — 카디널리티를 추정하는 확률적 자료구조인 HyperLogLog 다.
HyperLogLog 는 고정 12 KB 메모리로 수십억 카디널리티를 추정하는 비결정적 알고리즘이다.
...
Set 으로도 같은 일을 할 수 있지만, 1 억 unique 사용자라면 Set 는 수 GB 를 쓰지만 HLL 은 항상 12 KB 다[^V31].
...
각 멤버를 hash 하여 64 비트로 만든 뒤, 첫 14 비트로 register 인덱스 (0 ~ 16383) 를 결정하고 나머지 50 비트에서 최대 leading zero 수를 계산해 그 register 에 기록한다.
...
HLL 에 멤버를 추가하는 함수다[^V32].

```c
// src/hyperloglog.c (단순화)
...
다음은 지금까지 본 자료구조들을 직접 손으로 검증하는 §How 다.
...
각 응답이 본문에서 본 type marker (`+`, `$`) 로 시작하는 것이 보인다.
...
## 실습 ③: 코드 추적 — SET 명령이 어디까지 내려가는가

`SET foo bar` 한 줄이 Valkey 소스에서 어떤 함수들을 거치는지 추적한다.

### Step 1 — 명령어 테이블에서 등록 확인

`src/commands.def` (Valkey 의 자동 생성된 명령어 테이블) 에서 `SET` 명령이 어떤 함수에 매핑되는지 확인한다.
...
`setCommand` 가 `setGenericCommand` 를 부르고, 거기서 `setKey` 가 호출되며, 최종적으로 `dbAdd` 또는 `dbOverwrite` 가 호출된다.
...
SET foo bar (RESP)
  → setCommand (t_string.c)
    → setGenericCommand (t_string.c)
      → setKey (db.c)
        → dbAdd / dbOverwrite (db.c)
          → dictAdd (dict.c)
```

이 흐름을 다른 명령 (`HSET`, `ZADD` 등) 에도 적용해 보면 명령 → t_<type>.c → db.c → dict.c 라는 일관된 패턴이 보인다.
...
여기까지 따라온 사람은 Valkey 의 핵심 자료구조 7 가지를 공식 소스 코드와 함께 이해한 셈이다.
RESP → robj → encoding → C 자료구조의 5 단 사다리, 각 자료구조의 메모리 표현, 인코딩 전환 임계, 그리고 핵심 함수까지.
이 이해는 OSSCA 컨트리뷰션의 출발점이다.
명령을 추가하든 버그를 고치든, 모든 작업이 위 사다리의 어느 한 층에서 일어난다.
다음에는 해당 자료구조와 함수에 대해 코드를 수정해보고 테스트를 돌려보는 시간을 가져보고자 한다.
...
[^V1]: Linux Foundation — Announcing Valkey. [https://www.linuxfoundation.org/press/linux-foundation-launches-open-source-valkey-community](https://www.linuxfoundation.org/press/linux-foundation-launches-open-source-valkey-community)
[^V2]: Valkey 공식 — RESP protocol specification. [https://valkey.io/topics/protocol/](https://valkey.io/topics/protocol/)
[^V3]: Valkey 공식 — RESP3 specification. [https://github.com/valkey-io/valkey/blob/unstable/RESP3.md](https://github.com/valkey-io/valkey/blob/unstable/RESP3.md)
[^V4]: Valkey source — `src/server.h` 의 `redisObject` 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/server.h](https://github.com/valkey-io/valkey/blob/unstable/src/server.h)
[^V5]: Valkey 공식 — `OBJECT ENCODING` 명령. [https://valkey.io/commands/object-encoding/](https://valkey.io/commands/object-encoding/)
[^V6]: Valkey source — `src/server.h` 의 `OBJ_ENCODING_*` 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/server.h](https://github.com/valkey-io/valkey/blob/unstable/src/server.h)
[^V7]: Valkey source — `src/object.c` 의 `tryObjectEncoding()` 와 embstr → raw 전환 로직. [https://github.com/valkey-io/valkey/blob/unstable/src/object.c](https://github.com/valkey-io/valkey/blob/unstable/src/object.c)
[^V8]: [redisgate.kr](http://redisgate.kr/) — Redis 내부 String 자료구조 분석. [https://redisgate.kr/redis/configuration/internal_string.php](https://redisgate.kr/redis/configuration/internal_string.php)
[^V9]: Valkey source — `src/object.c` 의 `OBJ_ENCODING_EMBSTR_SIZE_LIMIT 44` 상수 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/object.c](https://github.com/valkey-io/valkey/blob/unstable/src/object.c)
[^V10]: Valkey source — `src/sds.h` 의 SDS 헤더 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/sds.h](https://github.com/valkey-io/valkey/blob/unstable/src/sds.h)
[^V11]: [redisgate.kr](http://redisgate.kr/) — Redis 내부 SDS 분석. [https://redisgate.kr/redis/configuration/internal_sds.php](https://redisgate.kr/redis/configuration/internal_sds.php)
[^V12]: Valkey source — `src/sds.h` 의 `sdshdr5/8/16/32/64` 5 가지 헤더 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/sds.h](https://github.com/valkey-io/valkey/blob/unstable/src/sds.h)
[^V13]: Valkey source — `src/sds.c` 의 `sdsMakeRoomFor()` 구현. [https://github.com/valkey-io/valkey/blob/unstable/src/sds.c](https://github.com/valkey-io/valkey/blob/unstable/src/sds.c)
[^V14]: Valkey source — `src/quicklist.h` 의 quicklist 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/quicklist.h](https://github.com/valkey-io/valkey/blob/unstable/src/quicklist.h)
[^V15]: Valkey source — `src/listpack.c` 와 listpack 디자인 문서. [https://github.com/valkey-io/valkey/blob/unstable/src/listpack.c](https://github.com/valkey-io/valkey/blob/unstable/src/listpack.c)
[^V16]: Valkey source — `src/quicklist.c` 의 `_quicklistInsert()` 구현. [https://github.com/valkey-io/valkey/blob/unstable/src/quicklist.c](https://github.com/valkey-io/valkey/blob/unstable/src/quicklist.c)
[^V17]: Valkey source — `src/t_hash.c` 의 Hash 명령 구현. [https://github.com/valkey-io/valkey/blob/unstable/src/t_hash.c](https://github.com/valkey-io/valkey/blob/unstable/src/t_hash.c)
[^V18]: Valkey 공식 docs — Hash data type. [https://valkey.io/topics/data-types/#hashes](https://valkey.io/topics/data-types/#hashes)
[^V19]: Valkey source — `src/t_hash.c` 의 `hashTypeConvertListpack()`. [https://github.com/valkey-io/valkey/blob/unstable/src/t_hash.c](https://github.com/valkey-io/valkey/blob/unstable/src/t_hash.c)
[^V20]: Valkey source — `src/t_set.c` 의 Set 명령 구현과 인코딩 분기. [https://github.com/valkey-io/valkey/blob/unstable/src/t_set.c](https://github.com/valkey-io/valkey/blob/unstable/src/t_set.c)
[^V21]: Valkey source — `src/intset.h` 와 `src/intset.c` 의 intset 구현. [https://github.com/valkey-io/valkey/blob/unstable/src/intset.h](https://github.com/valkey-io/valkey/blob/unstable/src/intset.h)
[^V22]: Valkey source — `src/t_set.c` 의 `setTypeAdd()`. [https://github.com/valkey-io/valkey/blob/unstable/src/t_set.c](https://github.com/valkey-io/valkey/blob/unstable/src/t_set.c)
[^V23]: Valkey source — `src/server.h` 의 `zset` 구조체 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/server.h](https://github.com/valkey-io/valkey/blob/unstable/src/server.h)
[^V24]: [redisgate.kr](http://redisgate.kr/) — Redis 내부 Sorted Set 과 skiplist 분석. [https://redisgate.kr/redis/configuration/internal_zset.php](https://redisgate.kr/redis/configuration/internal_zset.php)
[^V25]: Valkey source — `src/t_zset.c` 의 `zslInsert()`. [https://github.com/valkey-io/valkey/blob/unstable/src/t_zset.c](https://github.com/valkey-io/valkey/blob/unstable/src/t_zset.c)
[^V26]: [redisgate.kr](http://redisgate.kr/) — Redis 내부 dict 와 incremental rehashing. [https://redisgate.kr/redis/configuration/internal_dict.php](https://redisgate.kr/redis/configuration/internal_dict.php)
[^V27]: Valkey source — `src/dict.c` 의 `dictRehash()`. [https://github.com/valkey-io/valkey/blob/unstable/src/dict.c](https://github.com/valkey-io/valkey/blob/unstable/src/dict.c)
[^V28]: Valkey 공식 docs — Streams introduction. [https://valkey.io/topics/streams-intro/](https://valkey.io/topics/streams-intro/)
[^V29]: Valkey source — `src/stream.h` 의 stream 구조체 정의. [https://github.com/valkey-io/valkey/blob/unstable/src/stream.h](https://github.com/valkey-io/valkey/blob/unstable/src/stream.h)
[^V30]: Valkey source — `src/t_stream.c` 의 `streamAppendItem()`. [https://github.com/valkey-io/valkey/blob/unstable/src/t_stream.c](https://github.com/valkey-io/valkey/blob/unstable/src/t_stream.c)
[^V31]: Valkey 공식 docs — HyperLogLog. [https://valkey.io/topics/data-types/#hyperloglog](https://valkey.io/topics/data-types/#hyperloglog)
[^V32]: Valkey source — `src/hyperloglog.c` 의 `hllAdd()`. [https://github.com/valkey-io/valkey/blob/unstable/src/hyperloglog.c](https://github.com/valkey-io/valkey/blob/unstable/src/hyperloglog.c)
