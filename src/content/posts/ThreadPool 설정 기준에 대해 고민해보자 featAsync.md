...
> 
> 
> **4. 웬만하면 queueCapacity 와 maxPoolSize 는 건드리지 말자. 성능이슈도 있고, 더군다나 사용자의 요청수를 예측할 수 없기 때문이기도 하다.**
...
1. execute 방식은 작업 처리 중에 예외가 발생하면 해당 스레드가 종료되고 스레드 풀에서 제거한 뒤, 새로운 스레드를 생성하여 다른 작업을 처리한다. 또한 처리결과를 반환하지 않는다. 

2. 반대로 submit 은 작업 처리 중에 예외가 발생하더라도 스레드가 종료되지 않고 다음 작업에 사용된다. 또한 처리 결과를 Future<?> 로 반환한다. 
...
1. 처음 3개의 스레드(작업)이 스레드 풀에서 실행된다.

2. 이후 queue capacity 인 200까지 queue가 쌓일 때까지 실행한다. --아직까지 새로운 스레드가 생성되지 않고 스레드 풀의 스레드가 재사용된다.

3. queue 가 꽉 차게 되면, thread pool 은 maxPoolSize 인 5까지 늘어난다.

4. 이제 5개의 스레드(작업)이 스레드 풀에서 실행된다.
...
- [ThreadPoolExecutor javaDoc](https://docs.oracle.com/javase/6/docs/api/java/util/concurrent/ThreadPoolExecutor.html)
...
- [🧶 Java 에서 스레드 풀(Thread Pool) 을 사용해 보자 | 우아한테크 ](https://tecoble.techcourse.co.kr/post/2021-09-18-java-thread-pool/)

- [Configure the Spring ThreadPoolTaskExecutor.](https://codingtim.github.io/spring-threadpooltaskexecutor/)

- [ThreadPoolExecutor 동작 방식 및 주의 사항](https://blog.naver.com/bumsukoh/222175557879)
[[java] RejectExecutionHandler reject policy - 작업 큐(workQueue)가 full인 상태에서의 처리 정책](https://deep-dive-dev.tistory.com/11)

- [cached vs fixed thread pool | baeldung](https://www.baeldung.com/java-executors-cached-fixed-threadpool)

- [Spring - ThreadPoolExecutor reject policy 설정](https://codemanager.tistory.com/5)

- [spring boot async exception handling](https://velog.io/@stella6767/spring-boot-async-exception-handling)
