---
title: "DBRider 로 목업 테스트 데이터를 저장할 수 없던 이유, 그리고 해결 방법"
...
> 1. DBRider 는 DBUnit 을 통해 목업데이터 저장을 처리함
> 2. DBUnit 은 쿼리에 대한 호출만 처리, commit() 은 호출하지 않음
> 3. `ProxyConnection.close()` 처리 정책에 따라 데이터 롤백
> 4. 데이터 롤백을 방지하기 위해 테스트컨테이너 초기화 시, `HikariCP auto-commit : true` 설정
...
1. TestContext 로부터 데이터베이스에 대한 Connection 을 추출

2. 우리가 명시한 파일로부터 데이터들을 읽어서 IDataSet 구조체를 생성
...
1. 테스트 컨테이너에 연결이 설정되지 않은 것일 수도 있음
=> X 연결 자체는 정상적으로 수행되는 것을 확인할 수 있었다.

2. 테스트 컨테이너 버전과 DBRider 버전 문제일 수도 있음
=> X 모든 버전을 테스트해봤고, 더 나아가 코드를 보면 버전 간 동작원리나 과정이 크게 바뀌는 것은 없었다.

3. 어떤 이유로 DBUnit 의 Preparedstatement 가 전혀 작동하지 않는 것일 수도 있음
=> X DBRider의 모든 로직을 모킹하여 테스트해봤지만 모두 성공했다.

4. 데이터 형식(.json)이 잘못된 것일 수 있음
...
0. 사용하기 쉬울 것
1. 각기 다른 테스트케이스 별로 독립적으로 처리되어야 할 것
2. 어느 개발자가 실행되더라도 멱등하게 작동할 것
3. 목업 및 Assertion 이 알아보기 쉬울 것
