---
description: "NixOS 홈랩 서버를 Tailscale Exit Node로 설정하여 공공 WiFi에서 모든 Outbound 트래픽을 암호화된 WireGuard 터널로 라우팅하는 방법을 정리한다."
date: 2026-01-27
tags: [homelab, nix]
lang: ko
draft: false
...
공공 WiFi(카페, 공항, 호텔 등)는 다음과 같은 보안 위협에 노출된다:
...
Tailscale 네트워크의 특정 노드를 **모든 인터넷 트래픽의 출구(Exit)**로 지정하는 기능이다. 클라이언트의 트래픽이 해당 노드를 경유하여 인터넷으로 나간다.[^9]
...
Tailscale의 기본 `divert` 모드는 **netfilter/iptables를 우회**한다.[^10] 이 서버에서 그렇게 되면:
...
따라서 `nodivert`로 Tailscale이 netfilter 안에서 동작하게 강제하고, Exit Node에 필요한 MASQUERADE 규칙만 수동으로 추가한다.
