...
가장 안전함 |
| **keyboard-interactive** | 서버가 프롬프트를 보내고 사용자가 응답.

OTP, 2FA 구현에 사용 |
| **GSSAPI** | Kerberos 등 외부 인증 메커니즘 연동.
...
***nix 에서는 SSH 설치 시 fail2ban 이 같이 딸려나온다.

하지만 대부분 그렇지 않을 것이니 별도로 설치를 권장한다.
...
그냥 내 컴퓨터 사용해주십쇼 하는 거임

## 2.

Public Key 기반 SSH 연결
