---
title: "파사드 패턴을 통한 역할/책임 분리"
description: "현재 Service라는 이름으로 서로 다른 역할을 처리하고 있음Data에 대한 CRUD유스케이스 따른 흐름제어 &lt;&lt; 다른 도메인 서비스 의존아래와 같은 흐름으로 로직을 처리하고 있음컨트롤러 → 다른 도메인에 의존하여 흐름처리를 하는 유스케이스 → Data 에"
date: 2024-02-15
tags: [journal]
lang: ko
draft: false
...
`Service`라는 이름으로 **서로 다른 역할을 처리** 하는 게 주요원인이였다.

따라서 Service Layer 에서 두 가지 역할을 각각 분리하기로 하였다.

1. **_해당 도메인 역할_에 따른 단일 유스케이스** 처리
...
_**복합 유스케이스 처리가 아닌, 파사드 패턴을 통해 여러 곳에서 공통적으로 사용할 수 있는 객체**_ 라는 착각을 불러일으켰기 때문이다.
...
- 다음 세 가지 도메인이 긴밀하게 연관되어있다.
   ...
   - Buy(구매입찰) 
   - Point
  - <span style="color:yellowgreen">Point 는 User 내의 필드값으로 있어야한다고 판단</span>하였다. 
  - 왜냐하면 <span style="color:yellowgreen">User HAS-A Point 관계이기 때문</span>이다.
...
- API 당 Controller가 1:1이면 Overhead가 너무 크다고 생각한다.
    ...
    - Controller를 나누는 기준
        - `Query` 와 `Command`를 기준으로 나누어주기로 하였다.
          - `GET` → `Query`
