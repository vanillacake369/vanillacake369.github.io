...
const ONBOARDING_GROUPS: WhichKeyGroup[] = [
    {
        ...
        prefix: "",
        entries: [
            { key: "Ctrl/Cmd+/", description: "Which key" },
            { key: "/", description: "Search in page" },
            { key: "Ctrl/Cmd+K", description: "Search posts" },
...
export function init(): void {
    ...
    if (main) {
        new MutationObserver(invalidateHeadingCache).observe(main, {
            childList: true,
            subtree: false,
        });
    }
    ...
    document.addEventListener("vim:search-activated", () => {
        mode = "search";
    });
    showOnboarding();
}
